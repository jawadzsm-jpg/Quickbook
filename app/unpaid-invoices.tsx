...
export function UnpaidInvoices({ companyId, locationId, party, currency, paymentId, selectedInvoiceId, onSelect }: { companyId: number; locationId: number; party: string; currency: string; paymentId?: string; selectedInvoiceId: string; onSelect: (invoice: Invoice | null) => void }) {
  ...
  return <section className="overflow-hidden rounded-xl border bg-slate-50" aria-label="Unpaid invoices">
    <div className="border-b p-4"><h3 className="font-semibold">Unpaid invoices</h3><p className="mt-1 text-xs text-slate-500">Select a invoice to fill its remaining balance. You can enter a smaller amount for a partial payment. Earlier payments without a invoice link are not deducted here.</p></div>
    ...
    <div aria-live="polite">
      {!ready ? <p className="p-4 text-sm text-slate-500">Select a vendor, inventory and currency to see unpaid invoices.</p> : result.error ? <div className="flex items-center justify-between gap-3 p-4"><p role="alert" className="text-sm text-rose-600">{result.error}</p><Button type="button" variant="outline" onClick={() => { setResult({}); setAttempt((value) => value + 1); }}>Retry</Button></div> : !result.records ? <p className="p-4 text-sm text-slate-500">Loading unpaid invoices…</p> : !result.records.length ? <p className="p-4 text-sm text-slate-500">No invoices marked unpaid for this vendor, inventory and currency.</p> : <>
