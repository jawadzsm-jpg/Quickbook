...
export default async function Home() {
  ...
  if (!user) {
    let branding: { name: string; logoData: string; backgroundData: string; backgroundColor: string } | undefined;
    try {
      [branding] = await getDb().select({ name: companies.name, logoData: companies.logoData, backgroundData: companies.loginBackgroundData, backgroundColor: companies.loginBackgroundColor })
