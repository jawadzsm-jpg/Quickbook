...
export function PurchaseReturnSource({ companyId, locationId, party, currency, selectedBillId, disabled, onSelect }: { companyId: number; locationId: number; party: string; currency: string; selectedBillId?: string; disabled?: boolean; onSelect: (bill: PurchaseReturnBill | null) => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/records?${params}`, { cache: "no-store", signal: controller.signal }).then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error || "Could not load supplier bills"); return data; }).then((data) => { if (!controller.signal.aborted) setBills(data.records || []); }).catch(() => { if (!controller.signal.aborted) setBills([]); });
    return () => controller.abort();
  }, [companyId, locationId, party, currency, ready]);
  return <div className="space-y-2 sm:col-span-2"><Label>Original supplier bill *</Label><Select disabled={disabled || !ready} value={selectedBillId || undefined} onValueChange={(value) => onSelect(bills.find((bill) => String(bill.id) === value) || null)}><SelectTrigger className="w-full"><SelectValue placeholder={!party ? "Select a vendor first" : "Select the bill being returned"} /></SelectTrigger><SelectContent>{bills.map((bill) => <SelectItem key={bill.id} value={String(bill.id)}>{bill.number} · {bill.transactionDate} · {bill.lines.length} returnable line{bill.lines.length === 1 ? "" : "s"}</SelectItem>)}{ready && !bills.length ? <SelectItem value="none" disabled>No returnable bills found</SelectItem> : null}</SelectContent></Select><p className="text-xs text-slate-500">The return is linked to this bill. Only quantities not already returned are available.</p></div>;
}
