...
export function ItemLogisticsCenter({ companyId, companyName, locationId, locationName, canEdit }: { companyId: number; companyName: string; locationId: number; locationName: string; canEdit: boolean }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/item-logistics", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ companyId, locationId, records: changed }) });
      const data = await response.json();
  ...
  return <div className="space-y-5">
    <section className="rounded-xl border bg-white shadow-sm">
      ...
      <div className="max-h-[68vh] overflow-auto"><Table className="min-w-[1500px]"><TableHeader className="sticky top-0 z-10 bg-white"><TableRow><TableHead className="min-w-96">Specs</TableHead><TableHead className="min-w-60">HS Code</TableHead><TableHead className="min-w-40">COO</TableHead><TableHead className="min-w-48">Dimension</TableHead><TableHead>Length (cm)</TableHead><TableHead>Width (cm)</TableHead><TableHead>Height (cm)</TableHead><TableHead>Weight (kg)</TableHead></TableRow></TableHeader><TableBody>{loading ? <TableRow><TableCell colSpan={8} className="py-12 text-center text-slate-500">Loading item specifications…</TableCell></TableRow> : visible.length ? visible.map((record) => <TableRow key={record.id} className={dirtyIds.has(record.id) ? "bg-amber-50/60" : ""}><TableCell><p className="font-bold text-blue-700">{record.name} <span className="text-xs font-medium uppercase text-rose-500">{record.status}</span></p><p className="mt-1 max-w-xl text-xs font-semibold text-slate-700">{record.description || record.sku} <span className="font-mono text-rose-500">#{record.itemNumber || record.sku}</span></p></TableCell><TableCell><Input list="hs-code-options" value={record.hsCode} disabled={!canEdit} onChange={(event) => update(record.id, "hsCode", event.target.value)} placeholder="Select or enter HS code" /></TableCell><TableCell><Input list="country-origin-options" value={record.countryOfOrigin} disabled={!canEdit} onChange={(event) => update(record.id, "countryOfOrigin", event.target.value.toUpperCase())} placeholder="Country" /></TableCell><TableCell><Input value={record.dimensionText} disabled={!canEdit} onChange={(event) => update(record.id, "dimensionText", event.target.value)} placeholder="e.g. 11 × 35 × 55 cm" /></TableCell>{(["lengthCm", "widthCm", "heightCm", "weightKg"] as const).map((field) => <TableCell key={field}><Input type="number" min="0" step="0.01" value={record[field]} disabled={!canEdit} onChange={(event) => update(record.id, field, Number(event.target.value))} className="min-w-28" /></TableCell>)}</TableRow>) : <TableRow><TableCell colSpan={8} className="py-12 text-center text-slate-500">No matching inventory items found.</TableCell></TableRow>}</TableBody></Table></div>
