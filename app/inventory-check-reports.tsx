...
const lineStatus = (line: ReportLine): CheckStatus => line.companyQuantities.some((entry) => entry.countedQuantity !== null && Math.abs(entry.countedQuantity - entry.quantity) > 0.000001) ? "not-ok" : line.companyQuantities.length > 0 && line.companyQuantities.every((entry) => entry.countedQuantity !== null) ? "ok" : "pending";
...
export function InventoryCheckReports({ companyId, companyName, locationId, locationName, canManage, currentUserName }: { companyId: number; companyName: string; locationId: number; locationName: string; canManage: boolean; currentUserName: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) => (statusFilter === "all" || recordStatus(record) === statusFilter) && (!term || [record.id, record.memo, record.remarkSummary, record.creator, record.creatorEmail, record.locationName].some((value) => String(value ?? "").toLowerCase().includes(term))));
