...
export function PurchaseOrderReceiving({ orderId, companyId, onSaved }: { orderId: number; companyId: number; onSaved: () => void }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/records", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind: "transactions", type: "item receipt", companyId, locationId, purchaseOrderId: orderId, transactionDate: date, memo, lines }) });
      const result = await response.json();
