...
const vendorReportKeys = new Set(["supplier-quickreport", "supplier-open-balance", "vendor-statements", "ap-aging-summary", "ap-aging-detail", "vendor-balances", "supplier-balance-detail", "unpaid-bills-detail", "accounts-payable-graph", "supplier-transactions"]);
...
function WorkflowHome({ onNavigate, onWorkflow }: { onNavigate: (view: View) => void; onWorkflow: (type: string, target: View) => void }) {
  ...
  const companyActions: OverviewWorkflowAction[] = [
    ...
    { label: "Letterhead Generator", detail: "Design and print A4 letterheads", icon: FileText, view: "letterhead" },
...
function PurchaseCenter({ companies, companyId, locationId, onWorkspaceChange, onEdit, records, accounts, currency, loading, search, setSearch, onRefresh, onCreate, onDelete, onTransaction, onOpenDetail, onReport, reportLoading, canWrite, canDelete }: { companies: CompanyWorkspace[]; companyId: number; locationId: number; onWorkspaceChange: (companyId: number, locationId: number) => void; onEdit?: (record: DataRecord) => void; records: DataRecord[]; accounts: DataRecord[]; currency: string; loading: boolean; search: string; setSearch: (v: string) => void; onRefresh: () => void; onCreate: () => void; onDelete: (id: number) => void; onTransaction: (type: string) => void; onOpenDetail: (id: number) => void; onReport: (key: string) => void; reportLoading: boolean; canWrite: boolean; canDelete: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-white p-4" aria-label="Purchase order reports"><div><h3 className="font-semibold">Purchase Order Summary</h3><p className="text-sm text-slate-500">Review order status by supplier for this company and inventory.</p></div><Button type="button" variant="outline" disabled={reportLoading} onClick={() => onReport("purchase-order-summary")}><Table2 className="size-4" />Open report</Button></section>
...
function VendorCenter({ companyId, onEdit, onEditPurchaseReturn, records, accounts, currency, loading, search, setSearch, onRefresh, onCreateVendor, onDelete, onDeletePurchaseReturn, onTransaction, onReport, reportLoading, onOpenDetail, canWrite, canDelete }: { companyId: number; onEdit?: (record: DataRecord) => void; onEditPurchaseReturn?: (record: DataRecord) => void; records: DataRecord[]; accounts: DataRecord[]; currency: string; loading: boolean; search: string; setSearch: (v: string) => void; onRefresh: () => void; onCreateVendor: () => void; onDelete: (id: number) => void; onDeletePurchaseReturn?: (id: number) => void; onTransaction: (type: string) => void; onReport: (key: string, period?: { start: string; end: string }, options?: { locationId: number | null; currency: string; supplierId?: number }) => void; reportLoading: boolean; onOpenDetail: (id: number) => void; canWrite: boolean; canDelete: boolean }) {
  ...
  const [returnsState, setReturnsState] = useState<{ supplierId: number; rows: Array<{ id: number; number: string; transactionDate: string; total: number; currency: string; billNumber: string }> } | null>(null);
  const visibleSuppliers = records.filter((record) => supplierFilter === "active" ? record.status === "active" : supplierFilter === "open" ? Math.abs(Number(record.balance) || 0) > 0.005 : supplierFilter === "custom" ? [record.name, record.company, record.email, record.phone].some((value) => String(value || "").toLowerCase().includes(customFilter.toLowerCase())) : true);
  const selectedSupplier = visibleSuppliers.find((record) => record.id === selectedSupplierId) ?? visibleSuppliers[0];
  const currentSupplierId = selectedSupplier?.id;
  const currentSupplierBalance = selectedSupplier?.balance;
  const selectedSupplierName = String(selectedSupplier?.name || "Supplier");
  const supplierInitials = selectedSupplierName
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
    .map((part) => part[0])
  ...
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [companyId, currentSupplierId, currentSupplierBalance]);
  const selectedReport = (key: string) => { if (selectedSupplier) onReport(key, undefined, { locationId: 0, currency, supplierId: selectedSupplier.id }); };
  ...
  const actions = [
    ...
    { label: "Return Purchases", detail: "Return items against the original supplier bill", type: "vendor credit", icon: ArrowRightLeft },
    { label: "A/P Aging Summary", detail: "Review outstanding supplier balances by age", report: "ap-aging-summary", icon: Table2 },
  ];
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950" aria-label="Supplier Centre">
      <div className="flex flex-col gap-4 border-b border-slate-200 bg-slate-50/80 px-5 py-4 dark:border-slate-800 dark:bg-slate-900/60 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-center gap-3">
          <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300"><Building2 className="size-5" /></span>
          <div><h3 className="font-bold text-slate-950 dark:text-white">Supplier Centre</h3><p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Review supplier details, balances, reports, and purchase returns.</p></div>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          {supplierFilter === "custom" && <label className="grid gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300">Search suppliers<input className="h-10 min-w-56 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 dark:border-slate-700 dark:bg-slate-950 dark:text-white" value={customFilter} onChange={(event) => setCustomFilter(event.target.value)} placeholder="Name, email, or phone" /></label>}
          <label className="grid gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300">Show<select className="h-10 min-w-52 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 dark:border-slate-700 dark:bg-slate-950 dark:text-white" value={supplierFilter} onChange={(event) => setSupplierFilter(event.target.value)}><option value="all">All Suppliers</option><option value="active">Active Suppliers</option><option value="open">Suppliers with Open Balances</option><option value="custom">Custom Filter…</option></select></label>
        </div>
      </div>
      {selectedSupplier ? <div className="grid gap-5 p-5 lg:grid-cols-[280px_minmax(0,1fr)] lg:p-6">
        <aside className="h-fit rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60" aria-label="Supplier selector">
          <div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Supplier list</p><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{visibleSuppliers.length} {visibleSuppliers.length === 1 ? "supplier" : "suppliers"}</p></div><span className="grid size-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300"><Users className="size-4" /></span></div>
          <label className="grid gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300">Selected supplier<select className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-slate-900 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 dark:border-slate-700 dark:bg-slate-950 dark:text-white" value={selectedSupplier.id} onChange={(event) => setSelectedSupplierId(Number(event.target.value))}>{visibleSuppliers.map((record) => <option key={record.id} value={record.id}>{String(record.name)}</option>)}</select></label>
          <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 dark:border-emerald-500/20 dark:bg-emerald-500/10"><p className="text-xs font-semibold text-emerald-800 dark:text-emerald-300">Current balance</p><p className="mt-1 text-lg font-bold tabular-nums text-emerald-950 dark:text-emerald-100">{formatMoney(Number(selectedSupplier.balance) || 0, String(selectedSupplier.currency || currency))}</p></div>
        </aside>
        <div className="min-w-0 space-y-5">
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white dark:border-slate-800 dark:from-slate-900 dark:to-slate-950" aria-label="Supplier profile">
            <div className="flex flex-col gap-4 border-b border-slate-200 p-5 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-center gap-4"><span className="grid size-14 shrink-0 place-items-center rounded-2xl bg-emerald-600 text-lg font-bold text-white shadow-sm">{supplierInitials}</span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h4 className="truncate text-xl font-bold text-slate-950 dark:text-white">{selectedSupplierName}</h4><Badge className="capitalize border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300">{String(selectedSupplier.status || "active")}</Badge></div><p className="mt-1 truncate text-sm text-slate-500 dark:text-slate-400">{String(selectedSupplier.company || "Independent supplier")}</p></div></div>
              <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size="sm" disabled={reportLoading} onClick={() => selectedReport("supplier-quickreport")} className="bg-white dark:bg-slate-950"><FileBarChart2 className="size-4" />Quick report</Button><Button type="button" size="sm" disabled={reportLoading} onClick={() => selectedReport("supplier-open-balance")} className="bg-emerald-600 text-white hover:bg-emerald-700"><WalletCards className="size-4" />Open balance</Button></div>
            </div>
            <dl className="grid gap-px bg-slate-200 dark:bg-slate-800 sm:grid-cols-2">
              <div className="bg-white p-4 dark:bg-slate-950"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Company name</dt><dd className="mt-1.5 break-words text-sm font-medium text-slate-900 dark:text-slate-100">{String(selectedSupplier.company || "—")}</dd></div>
              <div className="bg-white p-4 dark:bg-slate-950"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Full name</dt><dd className="mt-1.5 break-words text-sm font-medium text-slate-900 dark:text-slate-100">{selectedSupplierName}</dd></div>
              <div className="bg-white p-4 dark:bg-slate-950"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Billed from</dt><dd className="mt-1.5 break-words text-sm font-medium text-slate-900 dark:text-slate-100">{String(selectedSupplier.billingName || selectedSupplier.country || "—")}</dd></div>
              <div className="bg-white p-4 dark:bg-slate-950"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Note</dt><dd className="mt-1.5 whitespace-pre-wrap break-words text-sm font-medium text-slate-900 dark:text-slate-100">{String(selectedSupplier.description || "—")}</dd></div>
            </dl>
          </section>
          <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950" aria-label="Purchase returns">
            <div className="flex items-center justify-between gap-3"><div><h5 className="font-bold text-slate-950 dark:text-white">Purchase returns</h5><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">View, edit, or delete this supplier&apos;s return transactions.</p></div><Badge variant="outline" className="shrink-0">{supplierReturns.length} {supplierReturns.length === 1 ? "return" : "returns"}</Badge></div>
            {returnsLoading ? <div className="mt-4 rounded-xl border border-dashed border-slate-300 p-5 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">Loading purchase returns…</div> : supplierReturns.length ? <div className="mt-4 space-y-2">{supplierReturns.map((entry) => <article key={entry.id} className="flex w-full flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-4 transition hover:border-emerald-200 hover:bg-emerald-50/40 dark:border-slate-800 dark:bg-slate-900/50 dark:hover:border-emerald-500/30 dark:hover:bg-emerald-500/5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm text-slate-950 dark:text-white">{entry.number}</strong><span className="text-xs text-slate-500 dark:text-slate-400">{entry.transactionDate}</span></div>{entry.billNumber ? <span className="mt-1 block text-xs text-slate-500 dark:text-slate-400">Original bill {entry.billNumber}</span> : null}</div><div className="flex items-center justify-between gap-2 sm:justify-end"><strong className="mr-2 text-sm tabular-nums text-slate-950 dark:text-white">{formatMoney(entry.total, entry.currency)}</strong><Button type="button" variant="ghost" size="icon" title="View" aria-label={`View purchase return ${entry.number}`} onClick={() => onOpenDetail(entry.id)} className="text-slate-500 hover:bg-emerald-100 hover:text-emerald-700 dark:text-slate-400 dark:hover:bg-emerald-500/10 dark:hover:text-emerald-300"><Eye className="size-4" /></Button>{onEditPurchaseReturn && <Button type="button" variant="ghost" size="icon" title="Edit" aria-label={`Edit purchase return ${entry.number}`} onClick={() => onEditPurchaseReturn(entry)} className="text-slate-500 hover:bg-sky-100 hover:text-sky-700 dark:text-slate-400 dark:hover:bg-sky-500/10 dark:hover:text-sky-300"><Pencil className="size-4" /></Button>}{onDeletePurchaseReturn && <Button type="button" variant="ghost" size="icon" title="Delete" aria-label={`Delete purchase return ${entry.number}`} className="text-rose-600 hover:bg-rose-50 hover:text-rose-700 dark:text-rose-400 dark:hover:bg-rose-500/10" onClick={() => { if (window.confirm(`Delete purchase return ${entry.number}? Stock, supplier balance, A/P and VAT postings will be reversed.`)) onDeletePurchaseReturn(entry.id); }}><Trash2 className="size-4" /></Button>}</div></article>)}</div> : <div className="mt-4 rounded-xl border border-dashed border-slate-300 p-6 text-center dark:border-slate-700"><ArrowRightLeft className="mx-auto size-5 text-slate-400" /><p className="mt-2 text-sm font-medium text-slate-700 dark:text-slate-300">No purchase returns</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Returns created for this supplier will appear here.</p></div>}
          </section>
        </div>
      </div> : <div className="m-5 rounded-xl border border-dashed border-slate-300 p-8 text-center dark:border-slate-700"><Users className="mx-auto size-6 text-slate-400" /><p className="mt-3 font-semibold text-slate-800 dark:text-slate-200">No suppliers match this filter</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Try another view or update your custom search.</p></div>}
    </section>
...
function ReportCenter({ companyId, locationId, baseCurrency, vendors, vendorCurrencies, memorisedReports, onOpen, onOpenMemorised, onDeleteMemorised, loading }: { companyId: number; locationId: number; baseCurrency: string; vendors: DataRecord[]; vendorCurrencies: string[]; memorisedReports: MemorisedReportRecord[]; onOpen: (key: string, options?: { currency: string; supplierId?: number }) => void; onOpenMemorised: (record: MemorisedReportRecord) => void; onDeleteMemorised: (record: MemorisedReportRecord) => void; loading: boolean }) {
  const [reportSearch, setReportSearch] = useState("");
  const [activeCategory, setActiveCategory] = useState<"All" | "Memorised Reports" | ReportCategory>("All");
  const [selectedVendorId, setSelectedVendorId] = useState("");
  const [vendorCurrency, setVendorCurrency] = useState(baseCurrency);
  const selectedVendor = vendors.find((vendor) => String(vendor.id) === selectedVendorId) ?? vendors[0];
  ...
  const openLibraryReport = (key: string) => {
    if (!vendorReportKeys.has(key)) return onOpen(key);
    if (vendorSelectionReportKeys.has(key) && !selectedVendor) return toast.error("Add or select a vendor before opening this report.");
    onOpen(key, { currency: vendorCurrency, supplierId: vendorSelectionReportKeys.has(key) ? selectedVendor?.id : undefined });
  };
    ...
    <section className="report-center-shell overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid min-h-[620px] lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <div className="report-library-panel min-w-0 bg-slate-50/70 p-5 sm:p-6">
          ...
          {activeCategory === "Vendors" && <section className="mb-6 rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-white p-4 shadow-sm dark:border-emerald-500/20 dark:from-emerald-500/10 dark:to-slate-950" aria-label="Vendor report options"><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><div className="flex items-center gap-2"><span className="grid size-9 place-items-center rounded-lg bg-emerald-600 text-white"><Building2 className="size-4" /></span><div><h4 className="font-bold text-slate-950 dark:text-white">Vendor report options</h4><p className="text-xs text-slate-500 dark:text-slate-400">Currency applies to all vendor reports; vendor selection applies to QuickReport and Open Balance.</p></div></div></div><div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300">Vendor for QuickReport / Open Balance<select className="h-10 min-w-56 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 dark:border-slate-700 dark:bg-slate-950 dark:text-white" value={selectedVendor?.id ?? ""} onChange={(event) => setSelectedVendorId(event.target.value)}>{vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{String(vendor.name)}</option>)}</select></label><label className="grid gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300">Transaction currency<select className="h-10 min-w-44 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 dark:border-slate-700 dark:bg-slate-950 dark:text-white" value={vendorCurrency} onChange={(event) => setVendorCurrency(event.target.value)}>{vendorCurrencies.map((code) => <option key={code} value={code}>{code}{code === baseCurrency ? " · Home" : " · Transactions"}</option>)}</select></label></div></div></section>}
          {activeCategory === "Memorised Reports" ? memorisedLibrary : <>{normalizedSearch && groupedMemorised.length ? <section className="mb-7"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2"><span className="h-5 w-1 rounded-full bg-emerald-500" /><h4 className="font-bold text-slate-900">Memorised Reports</h4></div><span className="text-xs font-medium text-slate-400">{filteredMemorised.length} saved</span></div>{memorisedLibrary}</section> : null}{groupedReports.length ? <div className="space-y-6">{groupedReports.map(({ category, reports: categoryReports }) => <section key={category}><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2"><span className="h-5 w-1 rounded-full bg-emerald-500" /><h4 className="font-bold text-slate-900">{category}</h4></div><span className="text-xs font-medium text-slate-400">{categoryReports.length} reports</span></div><div className="grid gap-3 xl:grid-cols-2">{categoryReports.map(([name, description, , key]) => <button key={name} type="button" disabled={loading || (vendorSelectionReportKeys.has(key) && !selectedVendor)} onClick={() => openLibraryReport(key)} className="report-center-card group flex min-h-24 items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 disabled:opacity-60"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-500 transition group-hover:bg-emerald-100 group-hover:text-emerald-700"><FileText className="size-5" /></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="block text-sm font-bold text-slate-900">{name}</span>{vendorSelectionReportKeys.has(key) && <Badge variant="outline" className="text-[10px]">Selected vendor</Badge>}</span><span className="mt-1 block text-xs leading-5 text-slate-500">{description}</span></span><ChevronRight className="size-5 shrink-0 text-slate-300 transition group-hover:translate-x-1 group-hover:text-emerald-500" /></button>)}</div></section>)}</div> : normalizedSearch && groupedMemorised.length ? null : <div className="report-center-empty rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center"><Search className="mx-auto size-7 text-slate-300" /><p className="mt-3 font-bold text-slate-800">No reports found</p><p className="mt-1 text-sm text-slate-500">Try a different report name or category.</p><Button variant="outline" size="sm" className="mt-4" onClick={resetFilters}>Show all reports</Button></div>} </>}
        </div>
      </div>
    </section>
...
function DocumentDialog({ onOpenInvoice, onReceiptSaved, onChequeNumberSaved, detail, companyName, baseCurrency, setup, canConvert, canEditCheque, onConvert, onClose }: { onOpenInvoice: (id: number) => void; onReceiptSaved: () => void; onChequeNumberSaved: (number: string, revision: string) => void; detail: TransactionDetail | null; companyName: string; baseCurrency: string; setup: CompanySetup; canConvert: boolean; canEditCheque: boolean; onConvert: (detail: TransactionDetail) => void; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[94vh] overflow-y-auto sm:max-w-[1400px]">
    ...
    {record.type === "cheque" ? <UaeBankCheque record={record} lines={detail.lines} journal={detail.journal} companyName={brandedName} revision={detail.revision || ""} canEditNumber={canEditCheque} onNumberSaved={onChequeNumberSaved} /> : usesSavedTemplate && savedTemplateMode ? <>
...
function ReportDialog({ onPurchaseOrder, onOpenReport, onCustomerDocument, onCustomer, onOpenSource, setup, loading, onStatementApply, onPricesSaved, report, companyName, inventoryName, memorised, saving, onMemorise, onClose }: { onPurchaseOrder: (supplier: string) => void; onOpenReport: (key: string) => void; onCustomerDocument: (customer: string, type: string) => void; onCustomer: (name: string, currency: string, overdue: boolean) => void; onOpenSource: (id: number) => void; setup: CompanySetup; loading: boolean; onStatementApply: (filters: { memo: string; customer: string; currency: string; statementDate: string; from: string; to: string }) => Promise<void>; onPricesSaved: () => Promise<void>; report: ReportData | null; companyName: string; inventoryName: string; memorised: boolean; saving: boolean; onMemorise: () => void; onClose: () => void }) {
  ...
  const stampReport = vendorReportKeys.has(report?.key || "");
  const statementStamp = report?.key === "vendor-statements";
  const stampLandscape = stampReport && !["ap-aging-summary", "vendor-statements"].includes(report?.key || "");
  const stampMaxLeft = stampLandscape ? 242 : 155;
  const stampMaxTop = stampLandscape ? 160 : 250;
  const [agingStamp, setAgingStamp] = useState(() => { const portrait = ["ap-aging-summary", "vendor-statements"].includes(report?.key || ""); const documentKey = report?.key === "vendor-statements" ? "statement" : report?.key === "supplier-quickreport" ? "supplier-quickreport" : report?.key === "supplier-open-balance" ? "supplier-open-balance" : "ap-aging-summary"; const saved = letterheadForDocument(setup.letterheadDesign, documentKey); return { show: Boolean(saved?.showStamp), left: Math.min(portrait ? 155 : 242, saved?.stampLeft ?? (portrait ? 145 : 225)), top: Math.min(portrait ? 250 : 160, saved?.stampTop ?? (portrait ? 230 : 155)) }; });
  ...
  const reportCell = (row: Record<string, string | number>, column: ReportData["columns"][number]) => {
    ...
    if (report.key === "purchase-order-summary" && column.key === "totalOrders" && Number(row.totalOrders) > 0) return <button type="button" className="font-semibold text-emerald-700 underline underline-offset-4 hover:text-emerald-500" aria-label={`Open purchase orders for ${row.supplier}`} onClick={() => onPurchaseOrder(String(row.supplier))}>{String(row.totalOrders)}</button>;
  ...
  const downloadReport = async (kind: "xlsx" | "csv" | "pdf") => {
    ...
    try {
      ...
      const data = kind === "csv" ? reportCsv(report, companyName, inventoryName, visibleRows) : kind === "xlsx" ? await reportWorkbook(report, companyName, inventoryName, visibleRows) : await reportPdf(report, companyName, inventoryName, visibleRows, stampReport && agingStamp.show && setup.stampData ? { data: setup.stampData, left: agingStamp.left, top: agingStamp.top } : undefined);
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className={`report-dialog ${report.key === "ap-aging-summary" ? "ap-aging-dialog" : stampLandscape ? "supplier-report-dialog" : ""} ${report.pnl ? "pnl-dialog" : ""} ${report.statement ? "customer-statement" : ""} max-h-[92dvh] min-w-0 overflow-y-auto sm:max-w-[calc(100%-2rem)]`}>
    ...
    {stampReport && <div className="flex flex-wrap items-center gap-3 rounded-lg border bg-slate-50 p-3 text-sm print:hidden"><label className="flex items-center gap-2 font-medium"><input type="checkbox" checked={agingStamp.show} disabled={!setup.stampData} onChange={(event) => setAgingStamp({ ...agingStamp, show: event.target.checked })} />Company stamp</label>{!setup.stampData && <span className="text-slate-500">Upload a stamp in Company Setup first.</span>}{agingStamp.show && setup.stampData && <><label className="flex items-center gap-2">Left (mm)<input type="number" min="0" max={stampMaxLeft} className="w-20 rounded border bg-white px-2 py-1" value={agingStamp.left} onChange={(event) => setAgingStamp({ ...agingStamp, left: Math.max(0, Math.min(stampMaxLeft, Number(event.target.value) || 0)) })} /></label><label className="flex items-center gap-2">Top (mm)<input type="number" min="0" max={stampMaxTop} className="w-20 rounded border bg-white px-2 py-1" value={agingStamp.top} onChange={(event) => setAgingStamp({ ...agingStamp, top: Math.max(0, Math.min(stampMaxTop, Number(event.target.value) || 0)) })} /></label><span className="text-slate-500">Drag the stamp to adjust its A4 position.</span></>}</div>}
    ...
    {report.statement && <><StatementFilters key={report.generatedAt} statement={report.statement} currency={report.currency} currencies={currencies} loading={loading} onApply={onStatementApply} /><StatementHeading statement={report.statement} currency={report.currency} company={setup} stamp={statementStamp ? { ...agingStamp, onMove: (left, top) => setAgingStamp({ ...agingStamp, left: Math.min(stampMaxLeft, left), top: Math.min(stampMaxTop, top) }) } : undefined} /></>}
    ...
    {report.key === "business-final" ? <BusinessFinalReport rows={report.rows} currency={report.currency} onOpenReport={onOpenReport} /> : report.financial ? <FinancialReport key={report.generatedAt} report={report as FinancialReportData} onOpen={onOpenSource} /> : report.pnl ? <ProfitLossReport key={report.generatedAt} report={report as PnlReport} company={companyName} loading={loading} onOpen={onOpenSource} onApply={(from, to) => onStatementApply({ from, to, currency: report.currency, customer: "", statementDate: "", memo: "" })} /> : report.activeCustomers ? <ActiveCustomersReport key={report.generatedAt} rows={report.rows} companyId={report.companyId!} canViewAccounts={report.activeCustomers.canViewAccounts} count={report.activeCustomers.count} onCustomer={onCustomer} onOpenSource={onOpenSource} /> : report.openBalance ? <CustomerOpenBalance key={report.generatedAt} data={report.openBalance} rows={report.rows} currency={report.currency} companyId={report.companyId!} loading={loading} onApply={onStatementApply} onOpen={onOpenSource} /> : <div data-report-columns={report.columns.length} className={`report-table min-w-0 rounded-xl border ${report.columns.length === 2 ? "report-table--compact" : report.columns.length > 6 ? "report-table--wide" : ""}`}><Table className="table-fixed"><colgroup>{report.columns.map((column, index) => <col key={column.key} style={{ width: `${columnWeights[index] / totalWeight * 100}%` }} />)}</colgroup><TableHeader><TableRow>{report.columns.map((column) => <TableHead key={column.key} className={column.type === "money" ? "text-right" : ""}>{column.label}</TableHead>)}</TableRow></TableHeader><TableBody>{visibleRows.length ? visibleRows.map((row, index) => <TableRow key={index}>{report.columns.map((column) => <TableCell key={column.key} className={column.type === "money" ? "text-right font-medium" : ""}>{reportCell(row, column)}</TableCell>)}</TableRow>) : <EmptyRow text={hideZeroQoh && report.rows.length ? "All zero-QOH rows are hidden." : "No posted data is available for this report."} columns={report.columns.length} />}</TableBody></Table></div>}
    {stampReport && !statementStamp && agingStamp.show && setup.stampData && <LetterheadStamp template={{ ...defaultLetterhead(), showStamp: true, stampLeft: agingStamp.left, stampTop: agingStamp.top }} company={setup} onMove={(left, top) => setAgingStamp({ ...agingStamp, left: Math.min(stampMaxLeft, left), top: Math.min(stampMaxTop, top) })} />}
...
function CompanySetupCenter({ setup, onSaved, canClearCompany, canSelectLoginBranding }: { canClearCompany: boolean; canSelectLoginBranding: boolean; setup: CompanySetup; onSaved: (setup: CompanySetup) => void | Promise<void> }) {
  const [form, setForm] = useState<CompanySetup>(setup);
  const [saving, setSaving] = useState(false);
  const [savingField, setSavingField] = useState<string | null>(null);
  const [setupTab, setSetupTab] = useState<"company" | "login">("company");
  const update = (field: keyof CompanySetup, value: string) => setForm((current) => ({ ...current, [field]: value }));
  const saveField = async (field: "logoData" | "rightLogoData" | "loginBranding" | "loginLogoData" | "loginCompanyLogoData" | "loginDisplayName" | "loginCopyrightYears" | "loginBackgroundColor" | "loginBackgroundData") => {
    setSavingField(field);
    try {
      const response = await fetch("/api/company-setup/branding", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ companyId: form.id, field, value: form[field] }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Could not save this setting.");
      const saved = data.record as CompanySetup;
      setForm((current) => ({ ...current, [field]: saved[field] }));
      await onSaved(saved);
      toast.success("Setting saved");
    } catch (error) { toast.error(error instanceof Error ? error.message : "Could not save this setting."); }
  ...
  const uploadLoginBackground = (file?: File) => {
    if (!file) return;
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type) || file.size > 2_000_000) return toast.error("Upload a PNG, JPG, or WebP background smaller than 2 MB.");
    const reader = new FileReader();
    reader.onload = () => update("loginBackgroundData", String(reader.result ?? ""));
    reader.onerror = () => toast.error("Could not read the selected background.");
    reader.readAsDataURL(file);
  };
  const uploadLoginLogo = (file?: File, field: "loginLogoData" | "loginCompanyLogoData" = "loginLogoData") => {
    if (!file) return;
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type) || file.size > 500_000) return toast.error("Upload a PNG, JPG, or WebP login image smaller than 500 KB.");
    const reader = new FileReader();
    reader.onload = () => update(field, String(reader.result ?? ""));
    reader.onerror = () => toast.error("Could not read the selected login image.");
    reader.readAsDataURL(file);
  };
  const save = async (event: FormEvent) => {
    event.preventDefault();
  ...
  return <form onSubmit={save} className="space-y-5">
    <div role="tablist" aria-label="Company setup sections" className="flex gap-2 border-b pb-2"><Button type="button" role="tab" aria-selected={setupTab === "company"} variant={setupTab === "company" ? "default" : "outline"} onClick={() => setSetupTab("company")}>Company setup</Button><Button type="button" role="tab" aria-selected={setupTab === "login"} variant={setupTab === "login" ? "default" : "outline"} onClick={() => setSetupTab("login")}>Login page</Button></div>
    {setupTab === "company" && <><div className="space-y-5">
      <section className="rounded-xl border bg-white p-5 shadow-sm"><div className="mb-5 flex items-center gap-3"><div className="brand-soft-icon grid size-10 place-items-center rounded-xl"><Building2 className="size-5" /></div><div><h2 className="font-bold">Company identity</h2><p className="text-sm text-slate-500">Used on invoices, bills, statements, and reports.</p></div></div><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2 md:col-span-2"><Label>Company Name *</Label><Input value={form.name} onChange={(event) => update("name", event.target.value)} required maxLength={120} /></div><div className="space-y-2 md:col-span-2"><Label>Left Logo</Label><div className="flex flex-wrap items-center gap-4 rounded-xl border border-dashed p-4">{form.logoData ? <Image src={form.logoData} alt="Company logo preview" width={96} height={64} unoptimized className="h-16 w-24 rounded-lg border bg-white object-contain p-1" /> : <div className="grid h-16 w-24 place-items-center rounded-lg bg-slate-100 text-slate-400"><ImageUp className="size-6" /></div>}<div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose logo<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => uploadLogo(event.target.files?.[0])} /></Label>{form.logoData ? <Button type="button" variant="outline" onClick={() => update("logoData", "")}>Remove</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("logoData")}>{savingField === "logoData" ? "Saving…" : "Save left logo"}</Button></div></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 500 KB</p></div><div className="space-y-2 md:col-span-2"><Label>Right Logo</Label><div className="flex flex-wrap items-center gap-4 rounded-xl border border-dashed p-4">{form.rightLogoData ? <Image src={form.rightLogoData} alt="Company logo preview" width={96} height={64} unoptimized className="h-16 w-24 rounded-lg border bg-white object-contain p-1" /> : <div className="grid h-16 w-24 place-items-center rounded-lg bg-slate-100 text-slate-400"><ImageUp className="size-6" /></div>}<div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose logo<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => uploadLogo(event.target.files?.[0], "rightLogoData")} /></Label>{form.rightLogoData ? <Button type="button" variant="outline" onClick={() => update("rightLogoData", "")}>Remove</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("rightLogoData")}>{savingField === "rightLogoData" ? "Saving…" : "Save right logo"}</Button></div></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 500 KB</p></div></div></section>
    ...
    <CompanyTemplateDesigner value={form.documentDesign} onChange={value => update("documentDesign", value)} setup={form} disabled={saving || savingField !== null} />
    {canClearCompany && <CompanyClearButton companyId={setup.id} companyName={setup.name} disabled={saving || savingField !== null} />}</>}
    {setupTab === "login" && <section role="tabpanel" aria-label="Login page settings" className="space-y-6 rounded-xl border bg-white p-5 shadow-sm">
      <div><h2 className="font-bold">Login page</h2><p className="mt-1 text-sm text-slate-500">Choose the upper image, company logo, and details for this company. Every active company can be chosen on the sign-in page.</p></div>
      {canSelectLoginBranding ? <div className="flex flex-wrap items-center gap-3"><label className="flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.loginBranding} onChange={(event) => setForm((current) => ({ ...current, loginBranding: event.target.checked }))} className="size-4 accent-blue-600" />Use this company as the default login page</label><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginBranding")}>{savingField === "loginBranding" ? "Saving…" : "Save selection"}</Button></div> : <p className="text-xs text-slate-500">A company administrator can choose the default company. This company is still available in the login page company selector.</p>}
      <div className="space-y-2"><Label>Upper image</Label><p className="text-xs text-slate-500">Displayed above the company logo on the login page. If empty, the default login upper image is used.</p><div className="flex flex-wrap items-center gap-4 rounded-xl border border-dashed p-4">{form.loginLogoData ? <Image src={form.loginLogoData} alt="Upper image preview" width={176} height={110} unoptimized className="h-24 w-40 rounded-lg bg-white object-contain" /> : <div className="grid h-24 w-40 place-items-center rounded-lg bg-slate-100 text-slate-400"><ImageUp className="size-7" /></div>}<div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose upper image<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { uploadLoginLogo(event.target.files?.[0]); event.target.value = ""; }} /></Label>{form.loginLogoData ? <Button type="button" variant="outline" onClick={() => update("loginLogoData", "")}>Remove</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginLogoData")}>{savingField === "loginLogoData" ? "Saving…" : "Save upper image"}</Button></div></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 500 KB</p></div>
      <div className="space-y-2"><Label>Login company logo</Label><p className="text-xs text-slate-500">Displayed below the upper image. If empty, the company left logo is used.</p><div className="flex flex-wrap items-center gap-4 rounded-xl border border-dashed p-4">{form.loginCompanyLogoData || form.logoData ? <Image src={form.loginCompanyLogoData || form.logoData} alt="Login company logo preview" width={176} height={110} unoptimized className="h-24 w-40 rounded-lg bg-white object-contain" /> : <div className="grid h-24 w-40 place-items-center rounded-lg bg-slate-100 text-slate-400"><ImageUp className="size-7" /></div>}<div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose company logo<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { uploadLoginLogo(event.target.files?.[0], "loginCompanyLogoData"); event.target.value = ""; }} /></Label>{form.loginCompanyLogoData ? <Button type="button" variant="outline" onClick={() => update("loginCompanyLogoData", "")}>Remove</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginCompanyLogoData")}>{savingField === "loginCompanyLogoData" ? "Saving…" : "Save company logo"}</Button></div></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 500 KB</p></div>
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2"><Label htmlFor="login-display-name">Display name</Label><div className="flex flex-wrap gap-2"><Input id="login-display-name" value={form.loginDisplayName} onChange={(event) => update("loginDisplayName", event.target.value)} placeholder={form.name || "Company name"} maxLength={120} className="min-w-44 flex-1" /><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginDisplayName")}>{savingField === "loginDisplayName" ? "Saving…" : "Save name"}</Button></div><p className="text-xs text-slate-500">Used when no company logo is set, and in the copyright line.</p></div>
        <div className="space-y-2"><Label htmlFor="login-copyright-years">Copyright years</Label><div className="flex flex-wrap gap-2"><Input id="login-copyright-years" value={form.loginCopyrightYears} onChange={(event) => update("loginCopyrightYears", event.target.value)} placeholder="1996-2021" maxLength={9} className="min-w-36 flex-1" /><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginCopyrightYears")}>{savingField === "loginCopyrightYears" ? "Saving…" : "Save years"}</Button></div><p className="text-xs text-slate-500">Copyright © {form.loginCopyrightYears || "1996-2021"} {form.loginDisplayName || form.name} All rights reserved.</p></div>
        <div className="space-y-2"><Label htmlFor="login-background-color">Background color</Label><div className="flex items-center gap-3"><Input id="login-background-color" type="color" value={form.loginBackgroundColor} onChange={(event) => update("loginBackgroundColor", event.target.value)} className="h-10 w-16 cursor-pointer p-1" /><span className="text-sm text-slate-500">{form.loginBackgroundColor}</span><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginBackgroundColor")}>{savingField === "loginBackgroundColor" ? "Saving…" : "Save color"}</Button></div></div>
        <div className="space-y-2"><Label>Background image</Label><div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose background<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { uploadLoginBackground(event.target.files?.[0]); event.target.value = ""; }} /></Label>{form.loginBackgroundData ? <Button type="button" variant="outline" onClick={() => update("loginBackgroundData", "")}>Remove</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginBackgroundData")}>{savingField === "loginBackgroundData" ? "Saving…" : "Save background"}</Button></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 2 MB. Choose another image any time.</p></div>
      </div>
      <div className="rounded-xl border bg-cover bg-center px-4 py-8 text-center" style={{ backgroundColor: form.loginBackgroundColor, ...(form.loginBackgroundData ? { backgroundImage: `linear-gradient(rgb(243 246 250 / 20%), rgb(243 246 250 / 20%)), url("${form.loginBackgroundData}")` } : {}) }}>
        <div className="mx-auto max-w-sm">{form.loginLogoData ? <Image src={form.loginLogoData} alt="Upper login image preview" width={260} height={155} unoptimized className="mx-auto max-h-36 w-auto max-w-full object-contain" /> : null}{form.loginCompanyLogoData || form.logoData ? <Image src={form.loginCompanyLogoData || form.logoData} alt="Login company logo preview" width={260} height={95} unoptimized className="mx-auto mt-3 max-h-24 w-auto max-w-full object-contain" /> : <div className="flex flex-col items-center gap-2"><ShieldCheck className="size-10 text-blue-600" /><strong>{form.loginDisplayName || form.name}</strong></div>}<div className="mt-4 rounded-lg bg-white p-5 shadow"><strong>Log in to start your session</strong><div className="mt-3 rounded border bg-blue-50 p-2 text-left text-xs text-slate-400">Email address</div><div className="mt-2 rounded border bg-blue-50 p-2 text-left text-xs text-slate-400">Password</div></div><p className="mt-4 rounded bg-white/70 p-2 text-xs">Copyright © {form.loginCopyrightYears || "1996-2021"} {form.loginDisplayName || form.name} All rights reserved.</p></div>
      </div>
    </section>}
  </form>;
}
