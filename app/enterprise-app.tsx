  ...
  return <form onSubmit={save} className="space-y-5">
    <div className="space-y-5">
      ...
      <section className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        {canSelectLoginBranding ? <label className="mt-4 flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.loginBranding} onChange={(event) => setForm((current) => ({ ...current, loginBranding: event.target.checked }))} className="size-4 accent-emerald-600" />Use this company on the login page</label> : <p className="mt-3 text-xs text-slate-500">An All Administrator selects which company appears on the shared login page.</p>}
