...
function VendorCenter({ companyId, onEdit, onEditPurchaseReturn, records, accounts, currency, loading, search, setSearch, onRefresh, onCreateVendor, onDelete, onDeletePurchaseReturn, onTransaction, onReport, reportLoading, onOpenDetail, canWrite, canDelete }: { companyId: number; onEdit?: (record: DataRecord) => void; onEditPurchaseReturn?: (record: DataRecord) => void; records: DataRecord[]; accounts: DataRecord[]; currency: string; loading: boolean; search: string; setSearch: (v: string) => void; onRefresh: () => void; onCreateVendor: () => void; onDelete: (id: number) => void; onDeletePurchaseReturn?: (id: number) => void; onTransaction: (type: string) => void; onReport: (key: string, period?: { start: string; end: string }, options?: { locationId: number | null; currency: string; supplierId?: number }) => void; reportLoading: boolean; onOpenDetail: (id: number) => void; canWrite: boolean; canDelete: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm" aria-label="Supplier Centre">
      <div className="flex flex-wrap items-end gap-3"><div className="min-w-44 flex-1"><h3 className="font-bold text-slate-900">Suppliers</h3><p className="mt-1 text-xs text-slate-500">Choose a supplier to view its information and reports.</p></div><label className="grid gap-1 text-xs font-medium text-slate-600">Show<select className="h-9 rounded-md border bg-white px-3 text-sm" value={supplierFilter} onChange={(event) => setSupplierFilter(event.target.value)}><option value="all">All Suppliers</option><option value="active">Active Suppliers</option><option value="open">Suppliers with Open Balances</option><option value="custom">Custom Filter…</option></select></label>{supplierFilter === "custom" && <label className="grid gap-1 text-xs font-medium text-slate-600">Filter by name or contact<input className="h-9 rounded-md border px-3 text-sm" value={customFilter} onChange={(event) => setCustomFilter(event.target.value)} placeholder="Supplier, email, phone…" /></label>}</div>
      {selectedSupplier ? <div className="mt-4 grid gap-5 border-t pt-4 lg:grid-cols-[minmax(210px,1fr)_2fr]"><label className="grid h-fit gap-1 text-xs font-medium text-slate-600">Selected supplier<select className="h-10 w-full rounded-md border bg-white px-3 text-sm" value={selectedSupplier.id} onChange={(event) => setSelectedSupplierId(Number(event.target.value))}>{visibleSuppliers.map((record) => <option key={record.id} value={record.id}>{String(record.name)}</option>)}</select></label><div className="min-w-0"><h4 className="font-semibold text-slate-900">{String(selectedSupplier.name)}</h4><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><div><dt className="text-slate-500">Company Name</dt><dd>{String(selectedSupplier.company || "—")}</dd></div><div><dt className="text-slate-500">Full Name</dt><dd>{String(selectedSupplier.name)}</dd></div><div><dt className="text-slate-500">Billed From</dt><dd>{String(selectedSupplier.billingName || selectedSupplier.country || "—")}</dd></div><div><dt className="text-slate-500">Note</dt><dd className="whitespace-pre-wrap break-words">{String(selectedSupplier.description || "—")}</dd></div></dl><div className="mt-4 flex flex-wrap items-center gap-2"><span className="mr-1 text-xs font-semibold text-slate-500">Reports for this supplier</span><Button type="button" variant="outline" size="sm" disabled={reportLoading} onClick={() => selectedReport("supplier-quickreport")}>QuickReport</Button><Button type="button" variant="outline" size="sm" disabled={reportLoading} onClick={() => selectedReport("supplier-open-balance")}>Open Balance</Button></div><div className="mt-5 border-t pt-4"><h5 className="text-sm font-semibold text-slate-900">Purchase Returns</h5>{returnsLoading ? <p className="mt-2 text-sm text-slate-500">Loading returns…</p> : supplierReturns.length ? <div className="mt-2 space-y-2">{supplierReturns.map((entry) => <article key={entry.id} className="flex w-full flex-wrap items-center justify-between gap-2 rounded-lg border p-3 text-sm"><span><strong>{entry.number}</strong> · {entry.transactionDate}{entry.billNumber ? <span className="block text-xs text-slate-500">Original bill {entry.billNumber}</span> : null}</span><div className="flex items-center gap-1"><strong className="mr-2">{formatMoney(entry.total, entry.currency)}</strong><Button type="button" variant="ghost" size="icon" title="View" aria-label={`View purchase return ${entry.number}`} onClick={() => onOpenDetail(entry.id)} className="text-slate-400 hover:text-emerald-600"><Eye className="size-4" /></Button>{onEditPurchaseReturn && <Button type="button" variant="ghost" size="icon" title="Edit" aria-label={`Edit purchase return ${entry.number}`} onClick={() => onEditPurchaseReturn(entry)}><Pencil className="size-4" /></Button>}{onDeletePurchaseReturn && <Button type="button" variant="ghost" size="icon" title="Delete" aria-label={`Delete purchase return ${entry.number}`} className="text-rose-600" onClick={() => { if (window.confirm(`Delete purchase return ${entry.number}? Stock, supplier balance, A/P and VAT postings will be reversed.`)) onDeletePurchaseReturn(entry.id); }}><Trash2 className="size-4" /></Button>}</div></article>)}</div> : <p className="mt-2 text-sm text-slate-500">No purchase returns for this supplier.</p>}</div></div></div> : <p className="mt-4 text-sm text-slate-500">No suppliers match this filter.</p>}
    </section>
