...
function CompanySetupCenter({ setup, onSaved, canClearCompany, canSelectLoginBranding }: { canClearCompany: boolean; canSelectLoginBranding: boolean; setup: CompanySetup; onSaved: (setup: CompanySetup) => void | Promise<void> }) {
  ...
  return <form onSubmit={save} className="space-y-5">
    ...
    {setupTab === "login" && <section role="tabpanel" aria-label="Login page settings" className="space-y-6 rounded-xl border bg-white p-5 shadow-sm">
      ...
      {canSelectLoginBranding ? <div className="flex flex-wrap items-center gap-3"><label className="flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.loginBranding} onChange={(event) => setForm((current) => ({ ...current, loginBranding: event.target.checked }))} className="size-4 accent-blue-600" />Use this company as the default login page</label><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginBranding")}>{savingField === "loginBranding" ? "Saving…" : "Save selection"}</Button></div> : <p className="text-xs text-slate-500">An All Administrator chooses the default company. This company is still available in the login page company selector.</p>}
