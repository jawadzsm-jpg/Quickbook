...
const emptyCompanySetup = (id = 0, name = "Company", baseCurrency = "AED"): CompanySetup => ({ id, name, baseCurrency, logoData: "", rightLogoData: "", loginBranding: false, loginLogoData: "", loginDisplayName: "", loginCopyrightYears: "1996-2021", loginBackgroundData: "", loginBackgroundColor: "#f3f6fa", documentDesign: "", stampData: "", addressLine1: "", addressLine2: "", city: "", country: "United Arab Emirates", phone: "", email: "", trn: "", bankName: "", bankAccountName: "", bankAccountNumber: "", bankIban: "", bankSwift: "", bankCurrency: baseCurrency, documentTemplate: "modern", documentColor: "#10b981" });
...
function CompanySetupCenter({ setup, onSaved, canClearCompany, canSelectLoginBranding }: { canClearCompany: boolean; canSelectLoginBranding: boolean; setup: CompanySetup; onSaved: (setup: CompanySetup) => void | Promise<void> }) {
  ...
  const saveField = async (field: "logoData" | "rightLogoData" | "loginBranding" | "loginLogoData" | "loginDisplayName" | "loginCopyrightYears" | "loginBackgroundColor" | "loginBackgroundData") => {
  ...
  const uploadLoginLogo = (file?: File) => {
    if (!file) return;
    ...
    const reader = new FileReader();
    reader.onload = () => update("loginLogoData", String(reader.result ?? ""));
  ...
  return <form onSubmit={save} className="space-y-5">
    ...
    {setupTab === "login" && <section role="tabpanel" aria-label="Login page settings" className="space-y-6 rounded-xl border bg-white p-5 shadow-sm">
      <div><h2 className="font-bold">Login page</h2><p className="mt-1 text-sm text-slate-500">Choose the image and details shown above the sign-in form. The login page is shared by all companies.</p></div>
      ...
      <div className="space-y-2"><Label>Login image</Label><p className="text-xs text-slate-500">Upload a separate image for above the login form. If empty, the company left logo and name are used.</p><div className="flex flex-wrap items-center gap-4 rounded-xl border border-dashed p-4">{form.loginLogoData ? <Image src={form.loginLogoData} alt="Login image preview" width={176} height={110} unoptimized className="h-24 w-40 rounded-lg bg-white object-contain" /> : <div className="grid h-24 w-40 place-items-center rounded-lg bg-slate-100 text-slate-400"><ImageUp className="size-7" /></div>}<div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose login image<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { uploadLoginLogo(event.target.files?.[0]); event.target.value = ""; }} /></Label>{form.loginLogoData ? <Button type="button" variant="outline" onClick={() => update("loginLogoData", "")}>Remove</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginLogoData")}>{savingField === "loginLogoData" ? "Saving…" : "Save login image"}</Button></div></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 500 KB</p></div>
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2"><Label htmlFor="login-display-name">Display name</Label><div className="flex flex-wrap gap-2"><Input id="login-display-name" value={form.loginDisplayName} onChange={(event) => update("loginDisplayName", event.target.value)} placeholder={form.name || "Company name"} maxLength={120} className="min-w-44 flex-1" /><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginDisplayName")}>{savingField === "loginDisplayName" ? "Saving…" : "Save name"}</Button></div><p className="text-xs text-slate-500">Used below the image when no separate login image is set, and in the copyright line.</p></div>
      ...
      <div className="rounded-xl border bg-cover bg-center px-4 py-8 text-center" style={{ backgroundColor: form.loginBackgroundColor, ...(form.loginBackgroundData ? { backgroundImage: `linear-gradient(rgb(243 246 250 / 20%), rgb(243 246 250 / 20%)), url("${form.loginBackgroundData}")` } : {}) }}>
        <div className="mx-auto max-w-sm">{form.loginLogoData ? <Image src={form.loginLogoData} alt="Login page preview" width={260} height={155} unoptimized className="mx-auto max-h-36 w-auto max-w-full object-contain" /> : <div className="flex flex-col items-center gap-2">{form.logoData ? <Image src={form.logoData} alt="Company logo preview" width={120} height={64} unoptimized className="h-16 w-28 object-contain" /> : <ShieldCheck className="size-10 text-blue-600" />}<strong>{form.loginDisplayName || form.name}</strong></div>}<div className="mt-4 rounded-lg bg-white p-5 shadow"><strong>Log in to start your session</strong><div className="mt-3 rounded border bg-blue-50 p-2 text-left text-xs text-slate-400">Email address</div><div className="mt-2 rounded border bg-blue-50 p-2 text-left text-xs text-slate-400">Password</div></div><p className="mt-4 rounded bg-white/70 p-2 text-xs">Copyright © {form.loginCopyrightYears || "1996-2021"} {form.loginDisplayName || form.name} All rights reserved.</p></div>
      </div>
    </section>}
  </form>;
}
