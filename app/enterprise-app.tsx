...
function ReportDialog({ onPurchaseOrder, onOpenReport, onCustomerDocument, onCustomer, onOpenSource, setup, loading, onStatementApply, onPricesSaved, report, companyName, inventoryName, memorised, saving, onMemorise, onClose }: { onPurchaseOrder: (supplier: string) => void; onOpenReport: (key: string) => void; onCustomerDocument: (customer: string, type: string) => void; onCustomer: (name: string, currency: string, overdue: boolean) => void; onOpenSource: (id: number) => void; setup: CompanySetup; loading: boolean; onStatementApply: (filters: { memo: string; customer: string; currency: string; statementDate: string; from: string; to: string }) => Promise<void>; onPricesSaved: () => Promise<void>; report: ReportData | null; companyName: string; inventoryName: string; memorised: boolean; saving: boolean; onMemorise: () => void; onClose: () => void }) {
  ...
  const [agingStamp, setAgingStamp] = useState(() => { const saved = letterheadForDocument(setup.letterheadDesign, "ap-aging-summary"); return { show: Boolean(saved?.showStamp), left: Math.min(155, saved?.stampLeft ?? 155), top: Math.min(250, saved?.stampTop ?? 230) }; });
  ...
  const downloadReport = async (kind: "xlsx" | "csv" | "pdf") => {
    ...
    try {
      ...
      const data = kind === "csv" ? reportCsv(report, companyName, inventoryName, visibleRows) : kind === "xlsx" ? await reportWorkbook(report, companyName, inventoryName, visibleRows) : await reportPdf(report, companyName, inventoryName, visibleRows, report.key === "ap-aging-summary" && agingStamp.show && setup.stampData ? { data: setup.stampData, left: agingStamp.left, top: agingStamp.top } : undefined);
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className={`report-dialog ${report.key === "ap-aging-summary" ? "ap-aging-dialog" : ""} ${report.pnl ? "pnl-dialog" : ""} ${report.statement ? "customer-statement" : ""} max-h-[92dvh] min-w-0 overflow-y-auto sm:max-w-[calc(100%-2rem)]`}>
    ...
    {report.key === "ap-aging-summary" && <div className="flex flex-wrap items-center gap-3 rounded-lg border bg-slate-50 p-3 text-sm print:hidden"><label className="flex items-center gap-2 font-medium"><input type="checkbox" checked={agingStamp.show} disabled={!setup.stampData} onChange={(event) => setAgingStamp({ ...agingStamp, show: event.target.checked })} />Company stamp</label>{!setup.stampData && <span className="text-slate-500">Upload a stamp in Company Setup first.</span>}{agingStamp.show && setup.stampData && <><label className="flex items-center gap-2">Left (mm)<input type="number" min="0" max="155" className="w-20 rounded border bg-white px-2 py-1" value={agingStamp.left} onChange={(event) => setAgingStamp({ ...agingStamp, left: Math.max(0, Math.min(155, Number(event.target.value) || 0)) })} /></label><label className="flex items-center gap-2">Top (mm)<input type="number" min="0" max="250" className="w-20 rounded border bg-white px-2 py-1" value={agingStamp.top} onChange={(event) => setAgingStamp({ ...agingStamp, top: Math.max(0, Math.min(250, Number(event.target.value) || 0)) })} /></label><span className="text-slate-500">Drag the stamp to adjust its A4 position.</span></>}</div>}
    ...
    {report.key === "ap-aging-summary" && agingStamp.show && setup.stampData && <LetterheadStamp template={{ ...defaultLetterhead(), showStamp: true, stampLeft: agingStamp.left, stampTop: agingStamp.top }} company={setup} onMove={(left, top) => setAgingStamp({ ...agingStamp, left: Math.min(155, left), top: Math.min(250, top) })} />}
