...
const emptyCompanySetup = (id = 0, name = "Company", baseCurrency = "AED"): CompanySetup => ({ id, name, baseCurrency, logoData: "", rightLogoData: "", loginBranding: false, loginBackgroundData: "", loginBackgroundColor: "#020617", documentDesign: "", stampData: "", addressLine1: "", addressLine2: "", city: "", country: "United Arab Emirates", phone: "", email: "", trn: "", bankName: "", bankAccountName: "", bankAccountNumber: "", bankIban: "", bankSwift: "", bankCurrency: baseCurrency, documentTemplate: "modern", documentColor: "#10b981" });
...
function CompanySetupCenter({ setup, onSaved, canClearCompany, canSelectLoginBranding }: { canClearCompany: boolean; canSelectLoginBranding: boolean; setup: CompanySetup; onSaved: (setup: CompanySetup) => void | Promise<void> }) {
  ...
  const saveField = async (field: "logoData" | "rightLogoData" | "loginBranding" | "loginBackgroundColor" | "loginBackgroundData") => {
    ...
    try {
      ...
      await onSaved(saved);
      toast.success(`${field === "loginBranding" ? "Login page company" : field === "loginBackgroundColor" ? "Background color" : field === "loginBackgroundData" ? "Background image" : field === "rightLogoData" ? "Right logo" : "Left logo"} saved`);
  ...
  const uploadLoginBackground = (file?: File) => {
    if (!file) return;
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type) || file.size > 500_000) return toast.error("Upload a PNG, JPG, or WebP background smaller than 500 KB.");
    const reader = new FileReader();
  ...
  return <form onSubmit={save} className="space-y-5">
    <div className="space-y-5">
      ...
      <section className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">Use the left company logo above on the sign-in page. The login page is shared by all companies.</p>
        {canSelectLoginBranding ? <div className="mt-4 flex flex-wrap items-center gap-3"><label className="flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.loginBranding} onChange={(event) => setForm((current) => ({ ...current, loginBranding: event.target.checked }))} className="size-4 accent-emerald-600" />Use this company on the login page</label><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginBranding")}>{savingField === "loginBranding" ? "Saving…" : "Save selection"}</Button></div> : <p className="mt-3 text-xs text-slate-500">An All Administrator selects which company appears on the shared login page.</p>}
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2"><Label>Background image (optional)</Label><div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose image<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { uploadLoginBackground(event.target.files?.[0]); event.target.value = ""; }} /></Label>{form.loginBackgroundData ? <Button type="button" variant="outline" onClick={() => update("loginBackgroundData", "")}>Remove image</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginBackgroundData")}>{savingField === "loginBackgroundData" ? "Saving…" : "Save image"}</Button></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 500 KB. The color shows when no image is selected.</p></div>
        </div>
        <div className="mt-4 grid min-h-40 place-items-center rounded-xl border bg-cover bg-center p-4" style={{ backgroundColor: form.loginBackgroundColor, ...(form.loginBackgroundData ? { backgroundImage: `linear-gradient(rgb(2 6 23 / 30%), rgb(2 6 23 / 30%)), url("${form.loginBackgroundData}")` } : {}) }}><div className="flex items-center gap-3 rounded-xl bg-white p-4 shadow-lg">{form.logoData ? <Image src={form.logoData} alt="Login logo preview" width={64} height={48} unoptimized className="h-12 w-16 object-contain" /> : <ShieldCheck className="size-8 text-emerald-600" />}<span className="font-bold text-slate-950">{form.name || "Company"} Accounting</span></div></div>
      </section>
