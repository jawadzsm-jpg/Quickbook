...
const emptyCompanySetup = (id = 0, name = "Company", baseCurrency = "AED"): CompanySetup => ({ id, name, baseCurrency, logoData: "", rightLogoData: "", loginBranding: false, loginLogoData: "", loginCompanyLogoData: "", loginDisplayName: "", loginCopyrightYears: "1996-2021", loginBackgroundData: "", loginBackgroundColor: "#f3f6fa", documentDesign: "", stampData: "", addressLine1: "", addressLine2: "", city: "", country: "United Arab Emirates", phone: "", email: "", trn: "", bankName: "", bankAccountName: "", bankAccountNumber: "", bankIban: "", bankSwift: "", bankCurrency: baseCurrency, documentTemplate: "modern", documentColor: "#10b981" });
...
function CompanySetupCenter({ setup, onSaved, canClearCompany, canSelectLoginBranding }: { canClearCompany: boolean; canSelectLoginBranding: boolean; setup: CompanySetup; onSaved: (setup: CompanySetup) => void | Promise<void> }) {
  ...
  return <form onSubmit={save} className="space-y-5">
    ...
    {setupTab === "login" && <section role="tabpanel" aria-label="Login page settings" className="space-y-6 rounded-xl border bg-white p-5 shadow-sm">
      <div><h2 className="font-bold">Login page</h2><p className="mt-1 text-sm text-slate-500">Choose the upper image, company logo, and details shown above the sign-in form. The login page is shared by all companies.</p></div>
      {canSelectLoginBranding ? <div className="flex flex-wrap items-center gap-3"><label className="flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.loginBranding} onChange={(event) => setForm((current) => ({ ...current, loginBranding: event.target.checked }))} className="size-4 accent-blue-600" />Show this company on the login page</label><Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginBranding")}>{savingField === "loginBranding" ? "Saving…" : "Save selection"}</Button></div> : <p className="text-xs text-slate-500">An All Administrator selects which company appears on the shared login page.</p>}
      <div className="space-y-2"><Label>Upper image</Label><p className="text-xs text-slate-500">Displayed above the company logo on the login page.</p><div className="flex flex-wrap items-center gap-4 rounded-xl border border-dashed p-4">{form.loginLogoData ? <Image src={form.loginLogoData} alt="Upper image preview" width={176} height={110} unoptimized className="h-24 w-40 rounded-lg bg-white object-contain" /> : <div className="grid h-24 w-40 place-items-center rounded-lg bg-slate-100 text-slate-400"><ImageUp className="size-7" /></div>}<div className="flex flex-wrap gap-2"><Label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border bg-white px-3 text-sm font-medium hover:bg-slate-50"><ImageUp className="size-4" />Choose upper image<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => { uploadLoginLogo(event.target.files?.[0]); event.target.value = ""; }} /></Label>{form.loginLogoData ? <Button type="button" variant="outline" onClick={() => update("loginLogoData", "")}>Remove</Button> : null}<Button type="button" variant="outline" disabled={saving || savingField !== null} onClick={() => void saveField("loginLogoData")}>{savingField === "loginLogoData" ? "Saving…" : "Save upper image"}</Button></div></div><p className="text-xs text-slate-500">PNG, JPG, or WebP · maximum 500 KB</p></div>
