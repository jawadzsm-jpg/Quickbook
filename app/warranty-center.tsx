...
type Slip = {
  ...
  supplierId: number | null; purchaseBillId: number | null; supplierName: string; purchaseNumber: string; purchaseDate: string;
  returnedToSupplierDate: string; receivedFromSupplierDate: string;
...
const emptySlip = (stamp = false, left = 156, top = 242): Slip => ({
  customerId: 0, invoiceId: null, invoiceLineId: null, supplierId: null, purchaseBillId: null,
  supplierName: "", purchaseNumber: "", purchaseDate: "", returnedToSupplierDate: "", receivedFromSupplierDate: "",
  slipDate: new Date().toLocaleDateString("en-CA"),
...
export function WarrantyCenter({ companyId, company, canWrite }: { companyId: number; company: Company; canWrite: boolean }) {
  ...
  const [activeStatus, setActiveStatus] = useState<WarrantyStatus>("Under Process");
  ...
  const dirty = Boolean(saved) ? JSON.stringify(form) !== JSON.stringify(saved) : Boolean(form.customerId || form.supplierId || form.problem || form.brand || form.model || form.remarks);
  ...
  function checkPage() {
    ...
    if (table && footer && table.getBoundingClientRect().bottom > footer.getBoundingClientRect().top - 8) { toast.error("The slip reaches the A4 footer. Shorten the details before printing."); return false; }
    return true;
  }
  function print() {
  ...
  const supplierBills = purchaseBills.filter((bill) => bill.party.trim().toLocaleLowerCase() === selectedSupplier?.name.trim().toLocaleLowerCase());
  const filtered = slips.filter((slip) => (normalizeWarrantyStatus(slip.status) ?? slip.status) === activeStatus && [slip.number, slip.customerName, slip.customerCompany, slip.contactName, slip.invoiceNumber, slip.supplierName, slip.purchaseNumber, slip.brand, slip.model, slip.serialNumber, slip.status].some((value) => String(value || "").toLocaleLowerCase().includes(search.toLocaleLowerCase())));

  return <section className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-white p-5"><div><h2 className="text-lg font-bold">Warranty / RMA Slips</h2><p className="text-sm text-slate-500">Track supplier and customer returns with printable A4 service records</p></div><div className="flex gap-2"><Button type="button" variant="outline" onClick={backToList} disabled={!showForm}>RMA list</Button>{canWrite && <Button type="button" onClick={createSlip}><FilePlus2 className="size-4" />New RMA slip</Button>}</div></div>
    {!showForm ? <div className="rounded-xl border bg-white p-5">
      <div className="mb-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4" aria-label="RMA status">
        {warrantyStatuses.map((status) => <button key={status} type="button" aria-pressed={activeStatus === status} onClick={() => setActiveStatus(status)} className={`flex items-center justify-between gap-2 rounded-lg border px-4 py-3 text-left text-sm font-semibold transition-colors ${activeStatus === status ? "border-teal-700 bg-teal-50 text-teal-900" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}><span>{status}</span><span className="rounded-full bg-white px-2 py-0.5 text-xs tabular-nums text-slate-700">{slips.filter((slip) => normalizeWarrantyStatus(slip.status) === status).length}</span></button>)}
      </div>
      <label className="mb-4 flex max-w-sm items-center gap-2 rounded-md border px-3"><Search className="size-4 text-slate-400" /><Input className="border-0 shadow-none" placeholder="Search customer, supplier, bill or serial" value={search} onChange={(event) => setSearch(event.target.value)} /></label>
      <div className="overflow-x-auto"><table className="w-full min-w-[1100px] border-collapse text-sm"><thead className="bg-slate-50 text-left"><tr>{["Date", "Slip #", "Company", "Name", "Invoice #", "Brand / Model", "Supplier", "Returned to supplier", "Received back", "Status", "Remark", "Created by", "Open"].map((heading) => <th key={heading} className="border px-3 py-2">{heading}</th>)}</tr></thead><tbody>{filtered.map((slip) => <tr key={slip.id}><td className="border px-3 py-2">{slip.slipDate}</td><td className="border px-3 py-2 font-semibold">{slip.number}</td><td className="border px-3 py-2">{slip.customerCompany || slip.customerName}</td><td className="border px-3 py-2">{slip.contactName || slip.customerName}</td><td className="border px-3 py-2">{slip.invoiceNumber || "—"}</td><td className="border px-3 py-2">{[slip.brand, slip.model].filter(Boolean).join(" · ") || "—"}</td><td className="border px-3 py-2">{slip.supplierName || "—"}</td><td className="border px-3 py-2">{slip.returnedToSupplierDate || "—"}</td><td className="border px-3 py-2">{slip.receivedFromSupplierDate || "—"}</td><td className="border px-3 py-2"><select className={selectClass} value={inlineEdits[slip.id!]?.status ?? slip.status} disabled={!canWrite} onChange={(event) => setInlineEdits((old) => ({ ...old, [slip.id!]: { status: event.target.value, remarks: old[slip.id!]?.remarks ?? slip.remarks } }))}>{warrantyStatuses.map((status) => <option key={status}>{status}</option>)}</select></td><td className="border px-3 py-2"><Input value={inlineEdits[slip.id!]?.remarks ?? slip.remarks} disabled={!canWrite} maxLength={2000} onChange={(event) => setInlineEdits((old) => ({ ...old, [slip.id!]: { status: old[slip.id!]?.status ?? slip.status, remarks: event.target.value } }))} /></td><td className="border px-3 py-2">{slip.createdBy || "—"}</td><td className="border px-3 py-2"><div className="flex gap-1">{inlineEdits[slip.id!] && <Button type="button" size="sm" disabled={busy} onClick={() => void saveInline(slip)}><Save className="size-3" />Save</Button>}<Button type="button" size="sm" variant="outline" onClick={() => openSlip(slip)}><Pencil className="size-3" />Open / Print</Button></div></td></tr>)}</tbody></table>{!filtered.length && <p className="py-7 text-center text-sm text-slate-500">{loading ? "Loading RMA slips…" : `No ${activeStatus.toLowerCase()} slips match this search.`}</p>}</div>
    </div> : <>
      <div className="space-y-5 rounded-xl border bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-bold">{form.number || "New warranty / RMA slip"}</h3><p className="text-sm text-slate-500">Choose an invoice to link the customer and item automatically, or enter warranty details manually.</p></div><div className="flex gap-2">{canWrite && <Button type="button" disabled={busy} onClick={() => void save()}><Save className="size-4" />Save slip</Button>}<Button type="button" variant="outline" onClick={print}><Printer className="size-4" />Print A4</Button><Button type="button" variant="outline" disabled={busy} onClick={() => void pdf()}><Download className="size-4" />PDF A4</Button></div></div>
        ...
        <fieldset className="rounded-lg border border-slate-200 p-4"><legend className="px-2 text-sm font-bold text-slate-800">Supplier and purchase</legend>
          <div className="grid gap-4 md:grid-cols-2">
            <label className={fieldClass}>Supplier<select className={selectClass} value={form.supplierId || ""} disabled={!canWrite} onChange={(event) => { const id = Number(event.target.value) || null; const supplier = suppliers.find((row) => row.id === id); setForm((old) => ({ ...old, supplierId: id, supplierName: supplier?.name || "", purchaseBillId: null, purchaseNumber: "", purchaseDate: "", returnedToSupplierDate: id === old.supplierId ? old.returnedToSupplierDate : "", receivedFromSupplierDate: id === old.supplierId ? old.receivedFromSupplierDate : "", status: id !== old.supplierId && old.status === "Returned to Supplier" ? "Under Process" : old.status })); }}><option value="">Select supplier</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}{supplier.company ? ` · ${supplier.company}` : ""}</option>)}</select></label>
            <label className={fieldClass}>Purchase bill<select className={selectClass} value={form.purchaseBillId || ""} disabled={!canWrite || !form.supplierId} onChange={(event) => { const id = Number(event.target.value) || null; const bill = supplierBills.find((row) => row.id === id); setForm((old) => ({ ...old, purchaseBillId: id, purchaseNumber: bill?.number || "", purchaseDate: bill?.transactionDate || old.purchaseDate })); }}><option value="">No linked bill</option>{supplierBills.map((bill) => <option key={bill.id} value={bill.id}>{bill.number} · {bill.transactionDate}</option>)}</select></label>
            <label className={fieldClass}>Purchase date<Input type="date" value={form.purchaseDate} disabled={!canWrite || Boolean(form.purchaseBillId)} onChange={(event) => update("purchaseDate", event.target.value)} /></label>
            <label className={fieldClass}>Returned to supplier on<Input type="date" value={form.returnedToSupplierDate} disabled={!canWrite || !form.supplierId} onChange={(event) => setForm((old) => ({ ...old, returnedToSupplierDate: event.target.value, receivedFromSupplierDate: event.target.value ? old.receivedFromSupplierDate : "" }))} /></label>
          ...
          <p className="mt-3 text-xs text-slate-500">Selecting a purchase bill fills its purchase date. Record both handoff dates as the unit moves to and from the supplier.</p>
        </fieldset>
