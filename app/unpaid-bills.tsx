...
export function UnpaidBills({ companyId, locationId, party, currency, paymentId, selectedBillId, onSelect, selectedBillIds, onSelectMany }: { companyId: number; locationId: number; party: string; currency: string; paymentId?: string; selectedBillId: string; onSelect: (bill: Bill | null) => void; selectedBillIds?: number[]; onSelectMany?: (bills: Bill[]) => void }) {
  ...
  return <section className="overflow-hidden rounded-xl border bg-slate-50" aria-label="Unpaid bills">
    <div className="border-b p-4"><h3 className="font-semibold">Unpaid bills</h3><p className="mt-1 text-xs text-slate-500">{onSelectMany ? "Select bills to fill their combined remaining balance. Partial payments apply oldest-first." : "Select a bill to fill its remaining balance."} You can enter a smaller amount for a partial payment. Earlier payments without a bill link are not deducted here.</p></div>
    {(selectedBillId || Boolean(selectedBillIds?.length)) && <div className="px-4 pt-3"><Button type="button" size="sm" variant="outline" onClick={() => onSelectMany ? onSelectMany([]) : onSelect(null)}>Clear bill selection</Button></div>}
    <div aria-live="polite">
      {!ready ? <p className="p-4 text-sm text-slate-500">Select a vendor, inventory and currency to see unpaid bills.</p> : result.error ? <div className="flex items-center justify-between gap-3 p-4"><p role="alert" className="text-sm text-rose-600">{result.error}</p><Button type="button" variant="outline" onClick={() => { setResult({}); setAttempt((value) => value + 1); }}>Retry</Button></div> : !result.records ? <p className="p-4 text-sm text-slate-500">Loading unpaid bills…</p> : !result.records.length ? <p className="p-4 text-sm text-slate-500">No bills marked unpaid for this vendor, inventory and currency.</p> : <>
        <div className="max-h-64 overflow-auto"><Table><TableHeader><TableRow><TableHead>{onSelectMany ? <input type="checkbox" aria-label="Select all unpaid bills" checked={Boolean(result.records?.length) && result.records!.every(isSelected)} onChange={(event) => onSelectMany(event.target.checked ? result.records || [] : [])} className="size-4 accent-emerald-600" /> : "Select"}</TableHead><TableHead>Bill reference</TableHead><TableHead>Bill date</TableHead><TableHead>Due date</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Bill total ({currency})</TableHead><TableHead className="text-right">Remaining ({currency})</TableHead></TableRow></TableHeader><TableBody>{result.records.map((bill) => <TableRow key={bill.id} data-state={isSelected(bill) ? "selected" : undefined}><TableCell><input type={onSelectMany ? "checkbox" : "radio"} name="payment-bill" aria-label={`Select bill ${bill.number}`} checked={isSelected(bill)} onChange={() => choose(bill)} className="size-4 accent-emerald-600" /></TableCell><TableCell className="font-medium">{bill.number}</TableCell><TableCell>{bill.transactionDate}</TableCell><TableCell>{bill.dueDate || "—"}</TableCell><TableCell className="capitalize">{bill.status}</TableCell><TableCell className="text-right">{money(bill.total)}</TableCell><TableCell className="text-right font-semibold">{money(bill.remaining)}</TableCell></TableRow>)}</TableBody></Table></div>
