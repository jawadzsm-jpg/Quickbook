...
export async function GET(request: Request) {
  try {
    ...
    const [allTransactions, allContacts, allItems, ledger, journal, lines] = await Promise.all([
      ...
      db.select({ name: journalLines.accountName, debit: sum(journalLines.debit), credit: sum(journalLines.credit) }).from(journalLines).groupBy(journalLines.accountName).orderBy(asc(journalLines.accountName)),
      db.select({ date: journalEntries.entryDate, reference: journalEntries.reference, description: journalEntries.description, account: journalLines.accountName, debit: journalLines.debit, credit: journalLines.credit }).from(journalLines).innerJoin(journalEntries, eq(journalLines.journalEntryId, journalEntries.id)).orderBy(asc(journalEntries.entryDate), asc(journalLines.id)),
      db.select({ itemId: transactionLines.itemId, description: transactionLines.description, quantity: transactionLines.quantity, subtotal: transactionLines.subtotal, unitCost: transactionLines.unitCost, type: transactions.type, party: transactions.party }).from(transactionLines).innerJoin(transactions, eq(transactionLines.transactionId, transactions.id)),
    ]);
    ...
    const txRows = (types?: string[]) => allTransactions.filter((row) => !types || types.includes(row.type)).map((row) => ({ date: row.transactionDate, number: row.number, type: row.type, party: row.party, status: row.status, amount: row.total }));
    ...
    const groupTransactions = (types: string[]) => {
      ...
      allTransactions.filter((row) => types.includes(row.type)).forEach((row) => grouped.set(row.party, (grouped.get(row.party) ?? 0) + row.total));
    ...
    const groupLines = (types: string[]) => {
      ...
      lines.filter((line) => types.includes(line.type)).forEach((line) => {
        ...
        grouped.set(line.description, { quantity: old.quantity + line.quantity, amount: old.amount + line.subtotal, cost: old.cost + line.quantity * line.unitCost });
      });
    ...
    const aged = (type: "invoice" | "bill") => allTransactions.filter((row) => row.type === type && !["paid", "cleared"].includes(row.status)).map((row) => {
      ...
      return { name: row.party, current: age <= 0 ? row.total : 0, days30: age > 0 && age <= 30 ? row.total : 0, days60: age > 30 && age <= 60 ? row.total : 0, days90: age > 60 ? row.total : 0, total: row.total };
    });
    ...
    return Response.json({ report: { key, title, generatedAt: new Date().toISOString(), columns, rows } });
  } catch (error) {
