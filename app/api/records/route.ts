    ...
    if (payload.kind === "transactions") {
      ...
      return await withWriteTransaction(async () => {
        ...
        if (existing?.type === "cheque" && payload.editMode === "cheque-layout") {
          const allowed = new Set(["kind", "id", "companyId", "revision", "editMode", "chequeBankKey", "chequeOffsetX", "chequeOffsetY", "chequeAmountOffsetX", "chequeDateOffsetX"]);
          ...
          const alignment = { x: Number(payload.chequeOffsetX ?? existing.chequeOffsetX), y: Number(payload.chequeOffsetY ?? existing.chequeOffsetY), amountX: Number(payload.chequeAmountOffsetX ?? existing.chequeAmountOffsetX), dateX: Number(payload.chequeDateOffsetX ?? existing.chequeDateOffsetX) };
          if (["chequeOffsetX", "chequeOffsetY", "chequeAmountOffsetX", "chequeDateOffsetX"].some((key) => payload[key] !== undefined && typeof payload[key] !== "number") || !validChequeAlignment(uaeChequeLayout(chequeBankKey), existing.terms !== "bearer", alignment)) return Response.json({ error: "The print position must fit entirely on the selected cheque paper." }, { status: 400 });
          const updates = { chequeBankKey, chequeOffsetX: alignment.x, chequeOffsetY: alignment.y, chequeAmountOffsetX: alignment.amountX, chequeDateOffsetX: alignment.dateX };
