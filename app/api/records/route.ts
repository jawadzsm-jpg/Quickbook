    ...
    if (payload.kind === "transactions") {
      ...
      return await withWriteTransaction(async () => {
        ...
        if (existing?.type === "cheque" && payload.editMode === "cheque-layout") {
          const allowed = new Set(["kind", "id", "companyId", "revision", "editMode", "chequeBankKey", "chequeOffsetX", "chequeOffsetY", "chequeAmountOffsetX", "chequeDateOffsetX", "chequeCrossingOffsetX"]);
          ...
          const alignment = { x: Number(payload.chequeOffsetX ?? existing.chequeOffsetX), y: Number(payload.chequeOffsetY ?? existing.chequeOffsetY), amountX: Number(payload.chequeAmountOffsetX ?? existing.chequeAmountOffsetX), dateX: Number(payload.chequeDateOffsetX ?? existing.chequeDateOffsetX), crossingX: Number(payload.chequeCrossingOffsetX ?? existing.chequeCrossingOffsetX) };
          if (["chequeOffsetX", "chequeOffsetY", "chequeAmountOffsetX", "chequeDateOffsetX", "chequeCrossingOffsetX"].some((key) => payload[key] !== undefined && typeof payload[key] !== "number") || !validChequeAlignment(uaeChequeLayout(chequeBankKey), existing.terms !== "bearer", alignment)) return Response.json({ error: "The print position must fit entirely on the selected cheque paper." }, { status: 400 });
          const updates = { chequeBankKey, chequeOffsetX: alignment.x, chequeOffsetY: alignment.y, chequeAmountOffsetX: alignment.amountX, chequeDateOffsetX: alignment.dateX, chequeCrossingOffsetX: alignment.crossingX };
          const [record] = await db.update(transactions).set(updates).where(eq(transactions.id, id)).returning();
          await db.insert(auditLog).values({ companyId, action: "updated", entityType: "transaction", entityId: id, details: JSON.stringify({ actor: { id: authorization.id, email: authorization.email }, mode: "cheque-layout", before: { chequeBankKey: existing.chequeBankKey, chequeOffsetX: existing.chequeOffsetX, chequeOffsetY: existing.chequeOffsetY, chequeAmountOffsetX: existing.chequeAmountOffsetX, chequeDateOffsetX: existing.chequeDateOffsetX, chequeCrossingOffsetX: existing.chequeCrossingOffsetX }, after: updates }) });
