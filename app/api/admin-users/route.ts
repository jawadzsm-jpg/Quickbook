...
async function publicUser(user: typeof appUsers.$inferSelect, currentUserId: number, assignedIds?: number[]) {
  ...
  return {
    ...
    companyIds: userRole === "all_admin" ? [] : assignedIds ?? await companyIdsForUser(user.id),
  };
}
...
async function handleGET(request: Request) {
  ...
  const memberships = await getDb().execute(sql`SELECT user_id, company_id FROM app_user_companies`);
  const byUser = new Map<number, number[]>();
  for (const row of memberships.rows) {
    const ids = byUser.get(Number(row.user_id)) ?? [];
    ids.push(Number(row.company_id));
    byUser.set(Number(row.user_id), ids);
  }
  return Response.json({ users: await Promise.all(users.map((user) => publicUser(user, administrator.id, byUser.get(user.id) ?? []))), companies: companyRows });
}

async function writePOST(request: Request) {
  ...
  try {
    [created] = await getDb().insert(appUsers).values({ fullName, email, phone, whatsapp, avatarData, role: role as never, passwordHash: (await hashPassword(password)), mustChangePassword: true }).returning();
  ...
  return Response.json({ user: await publicUser(created, administrator.id) }, { status: 201 });
}

async function writePATCH(request: Request) {
  const administrator = await requireApiUser(request, true, true);
  if (administrator instanceof Response) return administrator;
  if (administrator.role !== "all_admin") return Response.json({ error: "All-Admin permission required." }, { status: 403 });
  const payload = await request.json() as Record<string, unknown>;
  ...
  if (targetRole === "all_admin" && target.active && (nextRole !== "all_admin" || payload.active === false) && (await activeAllAdminCount()) <= 1) return Response.json({ error: "At least one active All-Admin is required." }, { status: 409 });
  let assignedCompanies: number[] | undefined;
  ...
  if (payload.companyIds !== undefined || payload.role !== undefined) {
    ...
    assignedCompanies = nextRole === "all_admin" ? [] : companyIds;
  }
  ...
  if (payload.password !== undefined) {
    const password = String(payload.password);
    if (password.length < 12 || password.length > 128) return Response.json({ error: "Temporary password must contain 12 to 128 characters." }, { status: 400 });
    updates.passwordHash = (await hashPassword(password)); updates.mustChangePassword = true;
  }
  ...
  if (assignedCompanies) await replaceCompanies(id, assignedCompanies);
  ...
  return Response.json({ user: await publicUser(updated, administrator.id) });
}
...
async function handleDELETE(request: Request) {
  ...
  if (targetRole === "all_admin" && target.active && (await activeAllAdminCount()) <= 1) return Response.json({ error: "At least one active All-Admin is required." }, { status: 409 });
...
export const POST = apiRoute((request) => saveUser(request, writePOST));

export const PATCH = apiRoute((request) => saveUser(request, writePATCH));

export const DELETE = apiRoute(handleDELETE, { transaction: true });

async function saveUser(request: Request, write: (request: Request) => Promise<Response>) {
  const administrator = await requireApiUser(request, true, true);
  if (administrator instanceof Response) return administrator;
  if (administrator.role !== "all_admin") return Response.json({ error: "All-Admin permission required." }, { status: 403 });
  const response = await withWriteTransaction(() => write(request));
  if (!response.ok) return response;
  const result = await response.json();
  const payload = await request.json();
  let emailSent = false;
  let emailWarning = "";
  if (payload.sendEmail === true && typeof payload.password === "string") {
    try {
      await sendUserInvitation({ to: result.user.email, name: result.user.fullName, temporaryPassword: payload.password, roleLabel: roleNames[result.user.role as AppRole], loginUrl: process.env.NEXT_PUBLIC_APP_URL ?? new URL(request.url).origin });
      emailSent = true;
    } catch { emailWarning = "The user was saved, but the invitation could not be sent. Check the email settings."; }
  }
  return Response.json({ ...result, emailSent, emailWarning }, { status: response.status });
}
