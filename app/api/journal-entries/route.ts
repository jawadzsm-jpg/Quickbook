...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ entries: entries.map((entry) => { const entryLines = lines.filter((line) => line.journalEntryId === entry.id); return { ...entry, lines: entryLines, debit: round(entryLines.reduce((sum, line) => sum + Number(line.debit), 0)), credit: round(entryLines.reduce((sum, line) => sum + Number(line.credit), 0)), source: entry.transactionId ? "Transaction" : "Manual" }; }) });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(entryDate) || !reference || !description) return Response.json({ error: "Date, reference and description are required." }, { status: 400 });
