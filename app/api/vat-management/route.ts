...
async function calculateVat(companyId: number, locationId: number, periodStart: string, periodEnd: string) {
  ...
  const outputVat = round(lines.reduce((sum, line) => {
    const sign = line.type === "credit memo" ? -1 : ["invoice", "sales receipt", "statement charge"].includes(line.type) ? 1 : 0;
    return sum + sign * Number(line.vatAmount) * Number(line.exchangeRate);
  }, 0));
  const inputVat = round(lines.reduce((sum, line) => {
    const sign = line.type === "vendor credit" ? -1 : ["bill", "received item bill", "expense", "cheque", "credit card charge"].includes(line.type) ? 1 : 0;
    return sum + sign * Number(line.vatAmount) * Number(line.exchangeRate);
  }, 0));
...
async function getVatManagement(request: Request) {
  ...
  try {
    ...
    if (Number.isInteger(locationId) && locationId > 0) {
      ...
      if (!location) return Response.json({ error: "Select an active inventory in this company." }, { status: 400 });
    }
  ...
  try {
    ...
    if (action === "file") {
      ...
      return await withWriteTransaction(async () => {
        const tx = getDb();
        const [duplicate] = await tx.select({ id: vatReturns.id }).from(vatReturns).where(and(eq(vatReturns.companyId, companyId), eq(vatReturns.reference, reference))).limit(1);
        ...
        const [record] = await tx.insert(vatReturns).values({ companyId, locationId, periodStart, periodEnd, reference, outputVat: summary.outputVat, inputVat: summary.inputVat, adjustments: summary.adjustments, netVatDue: summary.netVatDue, filedByUserId: user.id }).returning();
        await tx.insert(auditLog).values({ companyId, action: "filed", entityType: "vat_return", entityId: record.id, details: `${reference}; ${periodStart} to ${periodEnd}; VAT due ${summary.netVatDue.toFixed(2)}` });
