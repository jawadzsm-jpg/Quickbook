  ...
  try {
    ...
    return await withWriteTransaction(async () => {
      ...
      if (selected.has("setup")) {
        [record] = await db.update(companies).set({ logoData: "", rightLogoData: "", loginBackgroundData: "", loginBackgroundColor: "#020617", stampData: "", documentDesign: "", addressLine1: "", addressLine2: "", city: "", country: "", phone: "", email: "", trn: "", bankName: "", bankAccountName: "", bankAccountNumber: "", bankIban: "", bankSwift: "", bankCurrency: company.baseCurrency, documentTemplate: "modern", documentColor: "#10b981" }).where(eq(companies.id, companyId)).returning();
      }
