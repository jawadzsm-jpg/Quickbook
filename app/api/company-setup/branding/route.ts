...
const imageFields = new Set(["logoData", "rightLogoData", "loginBackgroundData"]);
const fields = new Set([...imageFields, "loginBackgroundColor", "loginBranding"]);
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (imageFields.has(field) && value && (typeof value !== "string" || !/^data:image\/(png|jpeg|webp);base64,/.test(value) || value.length > 700_000)) {
      return Response.json({ error: "Upload a PNG, JPG, or WebP image smaller than 500 KB." }, { status: 400 });
    }
