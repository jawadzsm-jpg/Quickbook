...
export function UserRoleCenter() {
    ...
    try {
      const response = await fetch("/api/admin-users", {
        ...
        body: JSON.stringify({
          id: user.id,
          role: user.role,
          companyIds: user.role === "all_admin" ? [] : user.companyIds,
        }),
      });
      const data = await response.json();
  ...
  return <div className="space-y-5">
    <section className="rounded-xl border bg-white shadow-sm">
      ...
      <div className="divide-y">{users.map((user) => <div key={user.id} className="grid gap-4 p-5 lg:grid-cols-[minmax(220px,1fr)_220px_minmax(320px,1.5fr)_120px] lg:items-end">
        ...
        <Button disabled={savingId === user.id || user.isCurrent || !dirtyIds.has(user.id)} onClick={() => void saveUser(user)}><Save className="mr-2 size-4" />{savingId === user.id ? "Saving..." : "Save"}</Button>
      </div>)}</div>
    </section>
