...
export function CustomInvoiceTemplate({design,record,lines,setup,contact,target='screen',editable=false,onMoveElement,onResizeElement}:{design:DocumentDesign;record:Data;lines:Data[];setup:TemplateBranding;contact?:Data|null;target?:'screen'|'print';editable?:boolean;onMoveElement?:(key:string,x:number,y:number)=>void;onResizeElement?:(key:string,width:number,height:number)=>void}) {
 ...
 return <article className="custom-invoice" style={{fontFamily:design.font,fontSize:design.fontSize,color:'#111',background:'#fff',padding:20}}>
  ...
  <div className="ci-bottom"><div className={editableClass('footerText')} style={properties('footerText')} onPointerDown={e=>startInteraction('footerText',e)}>{textFooter.map(f=><div key={f.key} className={editableClass(`footer.${f.key}`)} onPointerDown={e=>startInteraction(`footer.${f.key}`,e)} style={properties(`footer.${f.key}`)}><strong>{f.label}</strong><p>{String(values[f.key]||'')}</p></div>)}</div><div className={editableClass('totals')} style={properties('totals')} onPointerDown={e=>startInteraction('totals',e)}>{footer.filter(f=>!['message','disclaimer'].includes(f.key)).map(f=><div key={f.key} className={editableClass(`footer.${f.key}`,'ci-total')} onPointerDown={e=>startInteraction(`footer.${f.key}`,e)} style={properties(`footer.${f.key}`)}><strong>{f.label} ({String(record.currency||'AED')})</strong><span>{money(values[f.key])}</span></div>)}</div></div>
 </article>;
}
