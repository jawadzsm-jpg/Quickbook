...
export function CustomInvoiceTemplate({design,record,lines,setup,contact,letterhead,target='screen',editable=false,selectedElement,onSelectElement,onOpenElementProperties,onMoveElement,onResizeElement,onMoveCustomBox,onResizeCustomBox}:{design:DocumentDesign;record:Data;lines:Data[];setup:TemplateBranding;contact?:Data|null;letterhead?:LetterheadTemplate;target?:'screen'|'print';editable?:boolean;selectedElement?:string;onSelectElement?:(key:string)=>void;onOpenElementProperties?:(key:string)=>void;onMoveElement?:(key:string,x:number,y:number)=>void;onResizeElement?:(key:string,width:number,height:number,x?:number,y?:number)=>void;onMoveCustomBox?:(id:string,left:number,down:number)=>void;onResizeCustomBox?:(id:string,width:number,height:number,left?:number,down?:number)=>void}) {
 ...
 return <article className="custom-invoice" style={{fontFamily:design.font,fontSize:design.fontSize,color:'#111',background:'#fff',padding:20,...(letterhead ? { minHeight: 750 } : {}),position:'relative',minWidth:0,maxWidth:'100%'}}>
