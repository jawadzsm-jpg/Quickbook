...
export function CustomInvoiceTemplate({design,record,lines,setup,contact,target='screen',editable=false,onMoveElement,onResizeElement}:{design:DocumentDesign;record:Data;lines:Data[];setup:TemplateBranding;contact?:Data|null;target?:'screen'|'print';editable?:boolean;onMoveElement?:(key:string,x:number,y:number)=>void;onResizeElement?:(key:string,width:number,height:number)=>void}) {
 ...
 return <article className="custom-invoice" style={{fontFamily:design.font,fontSize:design.fontSize,color:'#111',background:'#fff',padding:20,position:'relative',minWidth:0,maxWidth:'100%'}}>
  ...
  {design.customBoxes.filter(box=>box[target]).map(box=><div key={box.id} className="ci-custom-box" style={{position:'absolute',left:`${box.left/7.6}%`,top:box.down,width:`${box.width/7.6}%`,height:box.height,maxWidth:`${Math.max(0,(760-box.left)/7.6)}%`,padding:6,overflow:'hidden',whiteSpace:'pre-wrap',overflowWrap:'anywhere',fontSize:box.fontSize,fontWeight:box.bold?700:400,border:box.border?'1px solid #94a3b8':'none',background:'#fff',zIndex:5}}>{box.text}</div>)}
 </article>;
}