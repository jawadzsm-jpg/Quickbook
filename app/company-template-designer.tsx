...
export function CompanyTemplateDesigner({value,onChange,setup,disabled}:{value:string;onChange:(value:string)=>void;setup:TemplateBranding;disabled:boolean}) {
 ...
 const print=()=>{
  ...
  const pageRule=design.printerMode==='specified'?`@page{size:${printPageSize()};margin:${design.margin}mm}`:'';
  popup.document.write(`<!doctype html><html><head><title>Invoice template preview</title><style>${pageRule}html,body{margin:0;padding:0}.print-copy{break-after:page;position:relative}.print-copy:last-child{break-after:auto}.print-page-number{text-align:center;font:11px Arial,sans-serif;color:#475569;margin-top:8mm}</style></head><body>${copies}</body></html>`);
 ...
 return <section className="col-span-full space-y-5 rounded-xl border bg-white p-5">
  ...
  <fieldset disabled={disabled} className="space-y-5">
   ...
   </div><div className="min-w-0 space-y-3"><div className="flex gap-2"><Button type="button" variant={target==='screen'?'default':'outline'} onClick={()=>setTarget('screen')}>Screen preview</Button><Button type="button" variant={target==='print'?'default':'outline'} onClick={()=>setTarget('print')}>Print preview</Button></div><p className="text-xs text-slate-500">Drag any dashed box to move it. Drag its blue bottom-right handle to resize it. Table columns still use the Columns width controls.</p><div ref={preview} className="max-h-[650px] overflow-auto border bg-white"><CustomInvoiceTemplate design={design} target={target} setup={setup} editable onMoveElement={moveElement} onResizeElement={resizeElement} record={{number:'INV-SAMPLE',transactionDate:'2026-09-13',dueDate:'2026-10-13',party:'Sample Customer',currency:'AED',status:'open',subtotal:1000,vatAmount:50,total:1050,balance:1050,salesman:'Sales Representative'}} lines={[{id:1,sku:'ITEM-001',description:'Sample product',quantity:2,unitPrice:500,serialNumber:'SN-001\nSN-002',subtotal:1000,vatAmount:50,total:1050,comments:'Sample item comments'}]}/></div>{target==='print'&&<Button type="button" variant="outline" onClick={print}>Print / Save PDF preview</Button>}</div></div>
