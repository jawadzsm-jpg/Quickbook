...
import { defaultDocumentDesign, readDocumentDesign, type DocumentDesign, type DesignField } from '@/lib/document-design';
 ...
 return <section className="col-span-full space-y-5 rounded-xl border bg-white p-5">
  ...
  <fieldset disabled={disabled} className="space-y-5">
   ...
   <div className="grid gap-5 xl:grid-cols-2"><div className="min-w-0 space-y-4" role="tabpanel">
    ...
    {tab==='Layout'&&<><p className="text-sm">The left logo, centered company title, and right logo form the header. Use Header and Columns to reorder fields and change widths.</p><div className="grid grid-cols-2 gap-3">{number('logoWidth','Logo width (px)',40,240)}{number('logoHeight','Logo height (px)',30,150)}</div><Button type="button" variant="outline" onClick={()=>{setEditingId('');update({...structuredClone(defaultDocumentDesign),savedTemplates:design.savedTemplates});}}>Restore default template</Button></>}
   </div><div className="min-w-0 space-y-3"><div className="flex gap-2"><Button type="button" variant={target==='screen'?'default':'outline'} onClick={()=>setTarget('screen')}>Screen preview</Button><Button type="button" variant={target==='print'?'default':'outline'} onClick={()=>setTarget('print')}>Print preview</Button></div><div ref={preview} className="max-h-[650px] overflow-auto border bg-white"><CustomInvoiceTemplate design={design} target={target} setup={setup} record={{number:'INV-SAMPLE',transactionDate:'2026-09-13',dueDate:'2026-10-13',party:'Sample Customer',currency:'AED',status:'open',subtotal:1000,vatAmount:50,total:1050,balance:1050,salesman:'Sales Representative'}} lines={[{id:1,sku:'ITEM-001',description:'Sample product',quantity:2,unitPrice:500,serialNumber:'SN-001\nSN-002',subtotal:1000,vatAmount:50,total:1050,comments:'Sample item comments'}]}/></div>{target==='print'&&<Button type="button" variant="outline" onClick={print}>Print / Save PDF preview</Button>}</div></div>
