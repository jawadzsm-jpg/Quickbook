 ...
 return <section className="col-span-full space-y-5 rounded-xl border bg-white p-5">
  ...
  <fieldset disabled={disabled} className="space-y-5">
   ...
   <div className="grid gap-5 xl:grid-cols-2"><div className="min-w-0 space-y-4" role="tabpanel">
    ...
    {tab==='Layout'&&<><p className="text-sm">Drag an outlined box to move it. Drag the blue handle on the bottom-right corner to resize the box. The saved position and size apply to every document using this template. Use Properties → Position for exact L / H / W / D controls, or Boxes to add/remove custom text boxes.</p><Button type="button" variant="outline" onClick={addBox}>Add Text Box</Button><div className="grid grid-cols-2 gap-3">{number('logoWidth','Logo width (px)',40,240)}{number('logoHeight','Logo height (px)',30,150)}</div><Button type="button" variant="outline" onClick={()=>{setEditingId('');update({...structuredClone(defaultDocumentDesign),savedTemplates:design.savedTemplates});}}>Restore default template</Button></>}
   </div><div className="min-w-0 space-y-3"><div className="flex flex-wrap gap-2"><Button type="button" variant={target==='screen'?'default':'outline'} onClick={()=>setTarget('screen')}>Screen preview</Button><Button type="button" variant={target==='print'?'default':'outline'} onClick={()=>setTarget('print')}>Print preview</Button>{target==='print'&&<Button type="button" className="brand-primary-button" onClick={()=>print(true)}>Print / Save PDF — A4</Button>}</div><p className="text-xs text-slate-500">A4 PDF output uses the current orientation and margins. Use the mouse to drag any dashed box. Drag the blue bottom-right handle to resize L / H / W / D directly, including header, columns, footer, and added boxes.</p><div ref={preview} className="max-h-[650px] overflow-auto border bg-white"><CustomInvoiceTemplate design={design} target={target} setup={setup} editable selectedElement={selectedElement} onSelectElement={setSelectedElement} onOpenElementProperties={openProperties} onMoveElement={moveElement} onResizeElement={resizeElement} onMoveCustomBox={(id,left,down)=>updateBox(id,{left,down})} onResizeCustomBox={(id,width,height,left,down)=>updateBox(id,{width,height,...(left===undefined?{}:{left}),...(down===undefined?{}:{down})})} record={{number:'INV-SAMPLE',transactionDate:'2026-09-13',dueDate:'2025-01-31',party:'Sample Customer',currency:'AED',status:'open',subtotal:1000,vatAmount:50,total:1050,balance:1050,salesman:'Sales Representative'}} lines={[{id:1,sku:'ITEM-001',description:'Sample product',quantity:2,unitPrice:500,serialNumber:'SN-001\nSN-002',subtotal:1000,vatAmount:50,total:1050,comments:'Sample item comments'}]}/></div>{target==='print'&&<div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={()=>print(false)}>Print / Save PDF preview</Button><Button type="button" className="brand-primary-button" onClick={()=>print(true)}>Print / Save PDF — A4</Button></div>}</div></div>
