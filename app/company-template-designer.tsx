   ...
   <div className="grid gap-5 xl:grid-cols-2"><div className="min-w-0 space-y-4" role="tabpanel">
    ...
    {tab==='Basic'&&<div className="overflow-hidden rounded-md border border-[#789b14] bg-white shadow-sm">
     ...
     <div className="space-y-5 p-4">
      <section className="rounded-md border p-4"><p className="mb-3 text-xs font-bold uppercase tracking-wide text-slate-700">Selected Template</p><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold">{design.name}</p><p className="text-xs text-muted-foreground">Current company invoice template</p></div><Button type="button" variant="outline" onClick={()=>setTab('Templates')}>Manage Templates...</Button></div><div className="mt-3 grid gap-1 text-sm"><label>Template name<Input required value={design.name} maxLength={80} onChange={e=>update({name:e.target.value})}/></label><label className="grid gap-1">Document title<select className="h-10 rounded-md border bg-background px-3" value={design.title} onChange={e=>update({title:e.target.value})}><option value="Tax Invoice">Tax Invoice</option><option value="Credit Note">Credit Note</option><option value="Refund">Refund</option><option value="Sales Receipt">Sales Receipt</option><option value="Purchase Order">Purchase Order</option><option value="Purchase Return">Purchase Return</option><option value="Statement of Account">Statement</option><option value="Estimate">Estimate</option><option value="Sales Order">Sales Order</option><option value="Delivery Note">Delivery Note</option><option value="Packing List">Packing List</option><option value="Proforma Invoice">Proforma Invoice</option><option value="Payment Receipt">Payment Receipt</option><option value="Paid To">Paid To</option><option value="Warranty Received">Warranty Received</option><option value="Quotation">Quotation</option></select></label></div></section>
