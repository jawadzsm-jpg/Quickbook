...
type JournalEntry = { id: number; entryDate: string; reference: string; description: string; transactionId: number | null; source: "Manual" | "Transaction" | "Stock revaluation" | "Automatic"; canManage: boolean; revision: string; debit: number; credit: number; lines: JournalLine[] };
...
export function JournalEntryCenter({ companyId, companyName, locationId, locationName, currency, accounts, onPosted, onOpenSource, onRevaluation }: { companyId: number; companyName: string; locationId: number; locationName: string; currency: string; accounts: Account[]; onPosted: () => Promise<void>; onOpenSource: (id: number) => void; onRevaluation: () => void }) {
  ...
  function editEntry(entry: JournalEntry) {
    ...
    setLines(entry.lines.map((line) => ({ accountId: String(activeAccounts.find((account) => account.name === line.accountName)?.id ?? ""), debit: line.debit ? String(line.debit) : "", credit: line.credit ? String(line.credit) : "" })));
  ...
  async function saveEntry(event: FormEvent) {
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/journal-entries", { method: editing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ companyId, locationId, entryDate, reference, description, lines, ...(editing ? { id: editing.id, revision: editing.revision } : {}) }) });
      const data = await response.json();
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-xl border bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Reference</TableHead><TableHead>Description</TableHead><TableHead>Source</TableHead><TableHead className="text-right">Debit</TableHead><TableHead className="text-right">Credit</TableHead><TableHead className="w-56" /></TableRow></TableHeader><TableBody>
        {loading ? <TableRow><TableCell colSpan={7} className="py-12 text-center text-slate-500">Loading journal entries…</TableCell></TableRow> : entries.length ? entries.map((entry) => <TableRow key={entry.id}><TableCell className="whitespace-nowrap text-slate-500">{entry.entryDate}</TableCell><TableCell className="font-mono text-xs font-semibold">{entry.reference}</TableCell><TableCell className="max-w-md truncate">{entry.description}</TableCell><TableCell><Badge variant={entry.source === "Manual" ? "default" : "outline"}>{entry.source}</Badge></TableCell><TableCell className="text-right font-semibold">{money(entry.debit, currency)}</TableCell><TableCell className="text-right font-semibold">{money(entry.credit, currency)}</TableCell><TableCell><div className="flex items-center gap-1"><Button variant="ghost" size="icon" onClick={() => setSelected(entry)} aria-label={`Open journal ${entry.reference}`}><Eye className="size-4" /></Button>{entry.canManage ? <><Button variant="ghost" size="sm" onClick={() => editEntry(entry)}><Pencil className="size-4" />Edit</Button><Button variant="ghost" size="sm" className="text-rose-600" onClick={() => setDeleting(entry)}><Trash2 className="size-4" />Delete</Button></> : entry.transactionId ? <Button variant="ghost" size="sm" onClick={() => onOpenSource(entry.transactionId!)}>Open source</Button> : entry.source === "Stock revaluation" ? <Button variant="ghost" size="sm" onClick={onRevaluation}>Stock revaluation</Button> : null}</div></TableCell></TableRow>) : <TableRow><TableCell colSpan={7} className="py-12 text-center text-slate-500">No journal entries for this inventory.</TableCell></TableRow>}
      </TableBody></Table></div>
    </section>
    ...
    <Dialog open={editorOpen} onOpenChange={setEditorOpen}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-6xl"><DialogHeader><DialogTitle>{editing ? "Edit general journal entry" : "New general journal entry"}</DialogTitle><DialogDescription>Every entry must contain equal debits and credits. It will post immediately to the ledger and financial reports.</DialogDescription></DialogHeader><form onSubmit={saveEntry} className="space-y-5">
      ...
      <DialogFooter><Button type="button" variant="outline" onClick={() => setEditorOpen(false)}>Cancel</Button><Button type="submit" disabled={saving || !balanced} className="brand-primary-button">{saving ? "Saving…" : editing ? "Save changes" : "Post journal entry"}</Button></DialogFooter>
