...
export function StockRevaluation() {
  ...
  const skuLock = useSkuLock(selected.size ? { resource: "stock-revaluation", records: [...selected].sort((a, b) => a - b).map((id) => ({ id })) } : null);
  async function save() {
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/stock-revaluation", { method: "POST", headers: { "Content-Type": "application/json", ...skuLock.headers }, body: JSON.stringify({ records: rows.map((r) => ({ id: r.id, cost: Number(drafts[r.id]?.cost ?? r.cost), salesPrice: Number(drafts[r.id]?.price ?? r.salesPrice), expectedCost: r.cost, expectedPrice: r.salesPrice, expectedQuantity: r.quantity })) }) });
      const data = await response.json();
