...
export function StockRevaluation() {
  ...
  return <section className="overflow-hidden rounded-xl border bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4 border-b p-5"><div><h2 className="text-lg font-bold">Stock Revaluation</h2><p className="text-sm text-slate-500">Cost of goods and the latest purchase cost are unit costs in the company&apos;s home currency. Changing the new unit cost posts the stock value adjustment; quantities stay unchanged.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={saving || loading} onClick={() => void load()}>Refresh / discard edits</Button>{canEdit && <Button disabled={saving || loading || !selected.size} onClick={() => void save()}>{saving ? "Saving…" : `Save selected (${selected.size})`}</Button>}</div></div><div className="flex flex-wrap gap-4 p-4"><label className="flex items-center gap-2 text-sm">Inventory<select className="rounded-md border p-2" value={location} onChange={(e) => setLocation(e.target.value)}><option value="">All inventories</option>{locations.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label><label className="ml-auto flex items-center gap-2 text-sm">Search<Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Item, SKU, company or inventory" /></label></div><div className="max-h-[70vh] overflow-auto"><Table className="min-w-[1250px]"><TableHeader className="sticky top-0 z-10 bg-slate-100"><TableRow><TableHead><Checkbox disabled={!canEdit || saving || loading || !visible.length} aria-label="Select visible items" checked={!!visible.length && visible.every((r) => selected.has(r.id))} onCheckedChange={(checked) => setSelected((old) => { const next = new Set(old); visible.forEach((r) => { if (checked === true) next.add(r.id); else next.delete(r.id); }); return next; })} /></TableHead><TableHead className="w-[45%]">SPECS · Stock Revaluation</TableHead><TableHead>QTY</TableHead><TableHead>COST OF GOODS<br /><span className="text-[11px] font-normal">UNIT COST · HOME CURRENCY</span></TableHead><TableHead>PURCHASE COST<br /><span className="text-[11px] font-normal">LATEST GRN · HOME CURRENCY</span></TableHead><TableHead>NEW UNIT COST<br /><span className="text-[11px] font-normal">HOME CURRENCY</span></TableHead><TableHead>PRICE<br /><span className="text-[11px] font-normal">HOME CURRENCY</span></TableHead></TableRow></TableHeader><TableBody>{loading ? <TableRow><TableCell colSpan={7}>Loading inventories…</TableCell></TableRow> : visible.length ? visible.map((r) => <TableRow key={r.id} className="align-top"><TableCell><Checkbox disabled={!canEdit || saving} checked={selected.has(r.id)} onCheckedChange={(v) => toggle(r.id, v === true)} aria-label={`Select ${r.name} in ${r.locationName}`} /></TableCell><TableCell className="whitespace-normal"><p className="font-bold text-blue-700 underline">{r.name}</p><p className="mt-1 whitespace-normal break-words text-sm">{r.description}</p><p className="mt-2 text-xs text-slate-500">{r.companyName} · {r.locationName} · Home currency: <span className="font-bold text-slate-700">{r.homeCurrency}</span> · SKU {r.sku} · <span className="text-rose-600">#{r.itemNumber}</span></p></TableCell><TableCell className="font-bold">{r.quantity}</TableCell><TableCell className="whitespace-nowrap font-semibold">{money(r.cost, r.homeCurrency)}</TableCell><TableCell className="whitespace-nowrap font-semibold text-sky-700">{money(r.grnCost, r.homeCurrency)}</TableCell><TableCell>{moneyInput(r, "cost", drafts[r.id]?.cost ?? r.cost, "New unit cost")}</TableCell><TableCell>{moneyInput(r, "price", drafts[r.id]?.price ?? r.salesPrice, "Price")}</TableCell></TableRow>) : <TableRow><TableCell colSpan={7}>No matching stock found.</TableCell></TableRow>}</TableBody></Table></div></section>;
}
