...
export type DocumentDesign = { properties: Record<string, ElementProperties>; savedTemplates: SavedTemplate[]; enabled: boolean; name: string; title: string; font: 'Arial' | 'Georgia' | 'Verdana'; fontSize: number; titleSize: number; companySize: number; color: string; leftLogo: boolean; rightLogo: boolean; logoWidth: number; logoHeight: number; showCompany: boolean; showAddress: boolean; showPhone: boolean; showEmail: boolean; statusStamp: boolean; headers: DesignField[]; columns: DesignField[]; footer: DesignField[]; message: string; disclaimer: string; printerMode: 'default' | 'specified'; copies: number; paper: 'A4' | 'A3' | 'Letter' | 'Legal' | 'Tabloid' | 'Custom'; customPaperWidth: number; customPaperHeight: number; orientation: 'portrait' | 'landscape'; margin: number; printPageNumbers: boolean; printTrailingZeros: boolean; decimals: number };
