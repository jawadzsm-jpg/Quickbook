...
export type SessionUser = { id: number; fullName: string; email: string; avatarData: string; themeColor: string; role: AppRole; mustChangePassword: boolean };
...
async function findSessionUser(token: string | undefined): Promise<SessionUser | null> {
  ...
  const [row] = await getDb().select({
    id: appUsers.id, fullName: appUsers.fullName, email: appUsers.email, avatarData: appUsers.avatarData, themeColor: appUsers.themeColor, role: appUsers.role, mustChangePassword: appUsers.mustChangePassword,
  }).from(authSessions)
