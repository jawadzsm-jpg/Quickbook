...
export async function financialReport(companyId:number,locationId:number,currency:string,params:URLSearchParams,canViewAccounts:boolean) {
 ...
 const isPnl=(e:typeof entries[number])=>{const t=accountFor(e.account)?.type||'';return income.has(t)||expense.has(t);};
 const pnlValue=(e:typeof entries[number])=>income.has(accountFor(e.account)?.type||'')?e.credit-e.debit:e.debit-e.credit;
 if(key.startsWith('income-customer')||key.startsWith('expenses-supplier')||key==='income-expense-graph') {
  ...
  const details:Row[]=selected.filter(e=>inRange(e.date)&&isPnl(e)&&(key==='income-expense-graph'||(wantIncome?income:expense).has(accountFor(e.account)!.type))).map(e=>({...linked(e),amount:round(pnlValue(e)),section:income.has(accountFor(e.account)!.type)?'Income':'Expenses'}));
  ...
  const balances=(end:string)=>{
   const totals=new Map<string,{debit:number;credit:number}>();let earnings=0;
   for(const e of selected.filter(e=>!end||e.date<=end)){const a=accountFor(e.account);if(!a)continue;const b=totals.get(e.account)||{debit:0,credit:0};b.debit+=e.debit;b.credit+=e.credit;totals.set(e.account,b);if(isPnl(e))earnings+=e.credit-e.debit;}
   const rows:Row[]=chart.filter(a=>accountFor(a.name)&&(assets.has(a.type)||liabilities.has(a.type)||a.type==='Equity')).map(a=>{const b=totals.get(a.name)||{debit:0,credit:0};return {name:a.name,account:a.name,accountId:a.id,section:assets.has(a.type)?'Assets':liabilities.has(a.type)?'Liabilities':'Equity',debit:round(b.debit),credit:round(b.credit),amount:round((assets.has(a.type)?1:-1)*(b.debit-b.credit))};});
  ...
  result.title='Statement of Cash Flows';const bank=(e:typeof entries[number])=>accountFor(e.account)?.type==='Bank';
  ...
  for(const group of groups.values()){
   ...
   const categories=new Set(group.filter(e=>!bank(e)&&(e.debit||e.credit)).map(e=>{const t=accountFor(e.account)?.type;return !t?'Unclassified':t==='Fixed Asset'||t==='Other Asset'?'Investing':['Equity','Loan','Long Term Liability'].includes(t)?'Financing':'Operating';}));
  ...
  const control=(d:typeof docs[number])=>{const names=new Set((postingsByDocument.get(d.id)||[]).filter(e=>['Accounts Receivable','Accounts Payable'].includes(accountFor(e.account)?.type||'')).map(e=>e.account));return names.size===1?accountFor([...names][0]):undefined;};
  ...
  if(key==='cash-flow-forecast'){
   result.title='Cash Flow Forecast';let projected=round(selected.filter(e=>e.date<=asOf&&accountFor(e.account)?.type==='Bank').reduce((n,e)=>n+e.debit-e.credit,0));
